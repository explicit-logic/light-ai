import { ChatModelFunctionsDocumentationGenerator } from './chatWrappers/utils/ChatModelFunctionsDocumentationGenerator';
...
export abstract class ChatWrapper {
  ...
  public generateContextState({
    ...
    const texts = historyWithFunctions.map((item) => {
      if (item.type === 'system') return LlamaText(['system: ', LlamaText.fromJSON(item.text)]);
      if (item.type === 'user') return LlamaText(['user: ', item.text]);
      if (item.type === 'model') return LlamaText(['model: ', this.generateModelResponseText(item.response)]);
  ...
  public generateFunctionCallsAndResults(functionCalls: ChatModelFunctionCall[], useRawCall = true) {
    const calls: LlamaText[] = [];
    const results: LlamaText[] = [];
    const res: LlamaText[] = [];

    if (functionCalls.length === 0) return LlamaText([]);

    for (const functionCall of functionCalls) {
      if (useRawCall && functionCall.rawCall != null) calls.push(LlamaText.fromJSON(functionCall.rawCall));
      else calls.push(this.generateFunctionCall(functionCall.name, functionCall.params));
    ...
    for (let i = 0; i < calls.length; i++) {
      if (i > 0) res.push(LlamaText(this.settings.functions.parallelism.call.betweenCalls ?? ''));
    ...
    for (let i = 0; i < results.length; i++) {
      if (i > 0) res.push(LlamaText(this.settings.functions.parallelism.result?.betweenResults ?? ''));
  ...
  public generateFunctionCall(name: string, params: any): LlamaText {
    return LlamaText([
      ...
      params === undefined ? '' : jsonDumps(params),
  ...
  public generateFunctionCallResult(functionName: string, functionParams: any, result: any): LlamaText {
    function resolveParameters(text: string | LlamaText) {
      return LlamaText(text).mapValues((value) => {
    ...
    return LlamaText([
      ...
      result === undefined ? 'void' : jsonDumps(result),
      resolveParameters(this.settings.functions.result.suffix),
    ]);
  }

  public generateModelResponseText(modelResponse: ChatModelResponse['response'], useRawCall = true): LlamaText {
    const res: LlamaText[] = [];
    ...
    for (const response of modelResponse) {
      ...
      if (response.startsNewChunk) addFunctionCalls();
  ...
  public generateAvailableFunctionsSystemText(
  ...
  public addAvailableFunctionsSystemMessageToHistory(
    ...
    if (availableFunctions == null || availableFunctionNames.length === 0) return history;
  ...
  /** @internal */
  public static _getOptionConfigurationsToTestIfCanSupersedeJinjaTemplate(): Array<
    Record<string | symbol, any> | [testConfig: Record<string | symbol, any>, applyConfig: Record<string | symbol, any>]
  > {
