...
import { LlamaModel, type LlamaModelOptions } from '../evaluator/LlamaModel/LlamaModel';
import { DisposeGuard } from '../utils/DisposeGuard';
import { ThreadsSplitter } from '../utils/ThreadsSplitter.js';
import type { GbnfJsonSchema } from '../utils/gbnfJson/types';
import { getConsoleLogPrefix } from '../utils/getConsoleLogPrefix';
import { type LlamaClasses, getLlamaClasses } from '../utils/getLlamaClasses';
import type { BindingModule } from './AddonTypes.js';
import type { BuildGpu, BuildMetadataFile, LlamaGpuType } from './types';
import { LlamaLocks, LlamaLogLevel, LlamaLogLevelGreaterThanOrEqual } from './types';
import { MemoryOrchestrator, type MemoryReservation } from './utils/MemoryOrchestrator.js';
...
export class Llama {
  ...
  private constructor({
    ...
    this._threadsSplitter = new ThreadsSplitter(
      maxThreads ?? (this._gpu === false ? Math.max(defaultCPUMinThreadSplitterThreads, this._mathCores) : 0),
    );

    this._logLevel = this._debug ? LlamaLogLevel.debug : (logLevel ?? LlamaLogLevel.debug);
    this._logger = logger;
    ...
    if (loadedGpu == null || (loadedGpu === false && gpu !== false)) this._bindings.loadBackends(true);
  ...
  public get classes() {
    if (this._classes == null) this._classes = getLlamaClasses();
  ...
  public set logLevel(value: LlamaLogLevel) {
    ...
    if (value === this._logLevel || this._debug) return;
  ...
  public set logger(value: (level: LlamaLogLevel, message: string) => void) {
    ...
    if (value !== Llama.defaultConsoleLogger) this._nextLogNeedNewLine = false;
  }
  ...
  public async getSwapState(): Promise<{
    ...
    return {
      maxSize: maxSize === -1 ? Number.POSITIVE_INFINITY : maxSize,
      allocated: total,
      used: total - free,
    };
  }
  ...
  private _onAddonLog(level: number, message: string) {
    ...
    const nextLog = lastNewLineIndex < 0 ? '' : sourceMessage.slice(lastNewLineIndex + 1);

    if (currentLog !== '') this._callLogger(llamaLogLevel, currentLog);
    ...
    } else this._pendingLog = null;
  }

  /** @internal */
  private _dispatchPendingLogMicrotask() {
    this._logDispatchQueuedMicrotasks--;
    if (this._logDispatchQueuedMicrotasks !== 0) return;
  ...
  private _callLogger(level: LlamaLogLevel, message: string) {
    ...
    if (logMessageIsOnlyDots(message) && this._logger === Llama.defaultConsoleLogger) {
      ...
      try {
        ...
        if (LlamaLogLevelGreaterThanOrEqual(transformedLogLevel, this._logLevel)) this._logger(transformedLogLevel, message);
      } catch (err) {
  ...
  public static async _create({
    ...
    const swapOrchestrator = new MemoryOrchestrator(() => {
      ...
      if (maxSize === -1)
        return {
          ...
          unifiedSize: Number.POSITIVE_INFINITY,
        };
    ...
    if (gpu === false || vramPadding === 0) resolvedVramPadding = vramOrchestrator.reserveMemory(0);
    ...
    else resolvedVramPadding = vramOrchestrator.reserveMemory(vramPadding);
    ...
    else resolvedRamPadding = ramOrchestrator.reserveMemory(ramPadding);
    ...
    if (!skipLlamaInit) await llama._init();
...
function getTransformedLogLevel(level: LlamaLogLevel, message: string): LlamaLogLevel {
  if (level === LlamaLogLevel.warn && message.endsWith('the full capacity of the model will not be utilized')) return LlamaLogLevel.info;
  if (level === LlamaLogLevel.warn && message.startsWith('ggml_metal_init: skipping kernel_') && message.endsWith('(not supported)'))
    return LlamaLogLevel.log;
  if (level === LlamaLogLevel.warn && message.startsWith('ggml_cuda_init: GGML_CUDA_FORCE_') && message.endsWith(' no'))
    return LlamaLogLevel.log;
