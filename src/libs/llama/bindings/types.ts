import type { BinaryPlatform } from './utils/getPlatform';
import type { BinaryPlatformInfo } from './utils/getPlatformInfo';

export const buildGpuOptions = ['metal', 'cuda', 'vulkan', false] as const;
export type LlamaGpuType = 'metal' | 'cuda' | 'vulkan' | false;
export const nodeLlamaCppGpuOptions = ['auto', ...buildGpuOptions] as const;
...
export function parseNodeLlamaCppGpuOption(
  option: (typeof nodeLlamaCppGpuOptions)[number] | (typeof nodeLlamaCppGpuOffStringOptions)[number],
): BuildGpu | 'auto' {
  ...
  if (optionIsGpuOff(option)) return false;
  if (option === 'auto') return 'auto';

  if (buildGpuOptions.includes(option)) return option;
