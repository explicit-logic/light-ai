...
export const builtinLlamaCppRelease = 'latest';
...
export async function getBuildFolderNameForBuildOptions(buildOptions: BuildOptions) {
  ...
  if (buildOptions.gpu !== false) nameParts.push(makeStringSafeForPathName(buildOptions.gpu));
...
function makeStringSafeForPathName(str: string) {
  ...
  for (const char of str) {
    if (isCharacterSafe(char)) res += char;
    else res += `_${char.codePointAt(0)!.toString(32)}_`;
  }
...
function containsUnsafeCharacters(str: string) {
  for (const char of str) {
    if (!isCharacterSafe(char)) return true;
  }
...
function isCharacterSafe(char: string) {
  ...
  if (unicodeNumber == null) return false;

  if (unicodeNumber >= 'a'.codePointAt(0)! && unicodeNumber <= 'z'.codePointAt(0)!) return true;
  if (unicodeNumber >= 'A'.codePointAt(0)! && unicodeNumber <= 'Z'.codePointAt(0)!) return true;
  if (unicodeNumber >= '0'.codePointAt(0)! && unicodeNumber <= '9'.codePointAt(0)!) return true;
