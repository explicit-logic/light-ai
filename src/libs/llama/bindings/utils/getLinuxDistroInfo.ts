...
const osReleasePaths = ['/etc/os-release', '/usr/lib/os-release'] as const;
...
export async function isDistroAlpineLinux(linuxDistroInfo: LinuxDistroInfo) {
  return (
    linuxDistroInfo.id === 'alpine' ||
    linuxDistroInfo.name.toLowerCase().startsWith('alpine') ||
...
async function getOsReleaseInfo() {
  for (const osReleasePath of osReleasePaths) {
    try {
      if (!(await fs.pathExists(osReleasePath))) continue;
