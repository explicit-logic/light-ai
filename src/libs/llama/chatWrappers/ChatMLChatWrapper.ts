...
export class ChatMLChatWrapper extends ChatWrapper {
  ...
  public override generateContextState({
      ...
      resultItems.map(({ system, user, model }, index) => {
        ...
        return LlamaText([
          ...
          user.values.length === 0
            ? LlamaText([])
            : LlamaText([new SpecialTokensText('<|im_start|>user\n'), user, new SpecialTokensText('<|im_end|>\n')]),
          ...
          model.values.length === 0 && !isLastItem
            ...
            : LlamaText([
                ...
                isLastItem ? LlamaText([]) : new SpecialTokensText('<|im_end|>\n'),
              ]),
        ]);
      }),
    );
    ...
    return {
      contextText,
      stopGenerationTriggers: [LlamaText(new SpecialToken('EOS')), LlamaText(new SpecialTokensText('<|im_end|>')), LlamaText('<|im_end|>')],
    };
  }
}
