import { ChatWrapper, type ChatWrapperJinjaMatchConfiguration } from '../ChatWrapper.js';
import {
...
export class FunctionaryChatWrapper extends ChatWrapper {
    ...
    if (variation === 'v3')
      this.settings = {
        ...
        functions: {
          ...
          result: {
            prefix: LlamaText([new SpecialTokensText('<|start_header_id|>tool<|end_header_id|>\n\n')]),
  ...
  public override generateContextState({
    ...
    if (this.variation === 'v3') return this._generateContextStateV3({ chatHistory, availableFunctions, documentFunctionParams });
  ...
  private _generateContextStateV3({
    ...
    const contextText = LlamaText(
      historyWithFunctions.map((item, index) => {
        ...
        if (item.type === 'user') {
          return LlamaText([new SpecialTokensText('<|start_header_id|>user<|end_header_id|>\n\n'), item.text, new SpecialToken('EOT')]);
        }
        if (item.type === 'model') {
          if (isLastItem && item.response.length === 0)
            return LlamaText([new SpecialTokensText('<|start_header_id|>assistant<|end_header_id|>\n\n')]);
          ...
          const addPendingFunctions = () => {
            if (pendingFunctionResults.length === 0) return;
          ...
          for (let index = 0; index < item.response.length; index++) {
                ...
                LlamaText([
                  ...
                  isLastResponse && response === ''
                    ...
                    : LlamaText([
                        ...
                        response,
                        isLastItem && isLastResponse ? LlamaText([]) : new SpecialToken('EOT'),
                      ]),
                ]),
              );
            } else if (isChatModelResponseFunctionCall(response)) {
              if (response.startsNewChunk) addPendingFunctions();
              ...
              pendingFunctionCalls.push(
                response.rawCall != null
                  ...
                  : LlamaText([
                      ...
                      response.name,
                      new SpecialTokensText('\n'),
                      response.params === undefined ? '' : jsonDumps(response.params),
                    ]),
              );
            ...
            } else void (response satisfies never);
          }
    ...
    const textResponseStart = [LlamaText(new SpecialTokensText('>>>all\n')), LlamaText('>>>all\n')];
    ...
    const contextText = LlamaText(
      ...
      historyWithFunctions.map((item, index) => {
        ...
        if (item.type === 'model') {
          ...
          if (isLastItem && (res.length === 0 || typeof item.response[item.response.length - 1] !== 'string'))
            res.push(LlamaText([new SpecialTokensText('<|start_header_id|>assistant<|end_header_id|>\n\n')]));
    ...
    const contextText = LlamaText(
      ...
      historyWithFunctions.map((item, index) => {
        ...
        if (item.type === 'system') {
          ...
          return LlamaText([
            isFirstItem ? LlamaText([]) : new SpecialTokensText('\n'),
        ...
        if (item.type === 'model') {
              ...
              pendingFunctionCalls.push(
                response.rawCall != null
                  ...
                  : LlamaText([
                      ...
                      new SpecialTokensText('<|from|>assistant\n'),
                      new SpecialTokensText('<|recipient|>'),
                      response.name,
                      new SpecialTokensText('\n'),
                      new SpecialTokensText('<|content|>'),
                      response.params === undefined ? '' : jsonDumps(response.params),
          ...
          if (!isLastItem) res.push(LlamaText(new SpecialTokensText('<|stop|>')));
  ...
  public override generateAvailableFunctionsSystemText(
    ...
    if (availableFunctionNames.length === 0) return LlamaText([]);
  ...
  public static override _getOptionConfigurationsToTestIfCanSupersedeJinjaTemplate() {
    return [{ variation: 'v3' }, { variation: 'v2.llama3' }, { variation: 'v2' }] satisfies ChatWrapperJinjaMatchConfiguration<typeof this>;
  }
}
