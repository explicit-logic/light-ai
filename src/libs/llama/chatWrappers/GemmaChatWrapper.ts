...
import type { ChatWrapperGenerateContextStateOptions, ChatWrapperGeneratedContextState, ChatWrapperSettings } from '../types.js';
    ...
    function flush() {
      if (systemTexts.length > 0 || userTexts.length > 0 || modelTexts.length > 0) {
        ...
        if (systemText.values.length > 0) {
          if (userText.values.length === 0) userText = systemText;
          else userText = LlamaText([systemText, '\n\n---\n\n', userText]);
        }
        ...
        return LlamaText([
          ...
          model.values.length === 0 && !isLastItem
            ...
            : LlamaText([
                ...
                isLastItem ? LlamaText([]) : new SpecialTokensText('<end_of_turn>\n'),
              ]),
        ]);
      }),
    );
