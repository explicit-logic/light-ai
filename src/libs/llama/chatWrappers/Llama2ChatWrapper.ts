...
import type { ChatWrapperGenerateContextStateOptions, ChatWrapperGeneratedContextState } from '../types.js';
  ...
  public override generateContextState({
    ...
    for (const item of historyWithFunctions) {
      if (item.type === 'system') {
        ...
        if (currentAggregateFocus !== 'system' && currentAggregateFocus !== 'user') flush();
          ...
          system.values.length === 0 && user.values.length === 0
            ...
            : LlamaText([
                ...
                system.values.length === 0
                  ? LlamaText([])
                  : LlamaText([new SpecialTokensText('<<SYS>>\n'), system, new SpecialTokensText('\n<</SYS>>\n\n')]),
                user,
                new SpecialTokensText(' [/INST] '),
              ]),
          model,
