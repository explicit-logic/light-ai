...
export class Llama3_2LightweightChatWrapper extends ChatWrapper {
  ...
  public constructor(
    ...
    this.todayDate = todayDate == null ? null : todayDate instanceof Function ? todayDate : new Date(todayDate);
  ...
  public override addAvailableFunctionsSystemMessageToHistory(
    ...
    if (res.length >= 2 && res[0]!.type === 'system' && res[1]!.type === 'system') res.splice(1, 0, functionsSystemMessage);
    else
      res.unshift({
        type: 'system',
        text: this.generateAvailableFunctionsSystemText(availableFunctions, { documentParams }).toJSON(),
  ...
  public override generateContextState({
          ...
          system:
            systemTexts.length === 0
              ...
              : LlamaText.joinValues(
                  resultItems.length === 0 && this._specialTokensTextForPreamble ? LlamaText(new SpecialTokensText('\n\n')) : '\n\n',
                  systemTexts,
                ),
          user: userTexts.length === 0 ? null : LlamaText.joinValues('\n\n', userTexts),
          model: modelTexts.length === 0 ? null : LlamaText.joinValues('\n\n', modelTexts),
        });
    ...
    const contextText = LlamaText(
      ...
      resultItems.map((item, index) => {
        ...
        if (item.system != null) {
          res.push(
            LlamaText([new SpecialTokensText('<|start_header_id|>system<|end_header_id|>\n\n'), item.system, new SpecialToken('EOT')]),
          );
        }

        if (item.user != null) {
          res.push(LlamaText([new SpecialTokensText('<|start_header_id|>user<|end_header_id|>\n\n'), item.user, new SpecialToken('EOT')]));
        }
        ...
        if (item.model != null) {
          res.push(
            LlamaText([
              ...
              item.model,
              isLastItem ? LlamaText([]) : new SpecialToken('EOT'),
            ]),
          );
        }
  ...
  public prependPreambleToChatHistory(chatHistory: readonly ChatHistoryItem[]): readonly ChatHistoryItem[] {
    ...
    const formatMonthDate = (date: Date, timezone?: 'UTC') => {
      const today = this.todayDate instanceof Function ? this.todayDate() : (this.todayDate ?? new Date());

      if (today.getUTCMonth() === date.getUTCMonth() && today.getUTCFullYear() === date.getUTCFullYear()) return formatDate(date, timezone);
    ...
    if (this.cuttingKnowledgeDate != null) {
      const date = this.cuttingKnowledgeDate instanceof Function ? this.cuttingKnowledgeDate() : this.cuttingKnowledgeDate;

      lines.push(`Cutting Knowledge Date: ${formatMonthDate(date, 'UTC')}`);
    }

    if (this.todayDate != null) {
      const date = this.todayDate instanceof Function ? this.todayDate() : this.todayDate;
