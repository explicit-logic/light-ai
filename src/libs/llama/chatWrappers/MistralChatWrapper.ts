...
export class MistralChatWrapper extends ChatWrapper {
  ...
  public override generateContextState({
    ...
    const contextText = LlamaText(
      ...
      chunkedChatHistory.map(({ system, user, model }) => {
        return LlamaText([
          ...
          LlamaText.joinValues(
            '\n\n',
            [system, user].filter((item) => item.values.length > 0),
          ),
          new SpecialTokensText('[/INST]'),
          model,
      ...
      toolsText === '' ? '' : [new SpecialTokensText('[AVAILABLE_TOOLS]'), toolsText, new SpecialTokensText('[/AVAILABLE_TOOLS]')],
      chunkedLastInteraction.map(({ system, user, model }, index) => {
        const isLastItem = index === chunkedLastInteraction.length - 1;

        return LlamaText([
          new SpecialTokensText('[INST]'),
          isLastItem && LlamaText(systemMessage).values.length > 0 ? [systemMessage, '\n\n'] : '',
          LlamaText.joinValues(
            '\n\n',
            [system, user].filter((item) => item.values.length > 0),
          ),
          ...
          model,
          this._addSpaceBeforeEos ? ' ' : '',
          isLastItem ? LlamaText([]) : new SpecialToken('EOS'),
        ]);
      }),
    );
    ...
    return {
      contextText,
      stopGenerationTriggers: [LlamaText(new SpecialToken('EOS')), LlamaText('</s>')],
    };
  }

  public override generateInitialChatHistory({ systemPrompt }: ChatWrapperGenerateInitialHistoryOptions = {}): ChatHistoryItem[] {
  ...
  private _generateAvailableToolsText({
    ...
    if (availableFunctions == null || availableFunctionNames.length === 0) return '';
    ...
    const availableTools = availableFunctionNames.map((functionName) => {
      ...
      return {
        ...
        function: {
          name: functionName,
          description:
            functionDefinition?.description != null && functionDefinition.description.trim() !== ''
              ? functionDefinition.description
              : undefined,
          parameters: documentFunctionParams && functionDefinition?.params != null ? functionDefinition.params : undefined,
        },
      };
    });
  ...
  public static override _getOptionConfigurationsToTestIfCanSupersedeJinjaTemplate() {
    return [{ addSpaceBeforeEos: false }, { addSpaceBeforeEos: true }] satisfies ChatWrapperJinjaMatchConfiguration<typeof this>;
  }
}
