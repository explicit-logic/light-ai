...
} from './utils/chatHistoryFunctionCallMessageTemplate.js';
...
export class JinjaTemplateChatWrapper extends ChatWrapper {
  ...
  public constructor({
    ...
    if (template == null) throw new Error('template cannot be null');

    this.template = template;
    this.modelRoleName = modelRoleName;
    this.userRoleName = userRoleName;
    this.systemRoleName = systemRoleName;
    this.convertUnsupportedSystemMessagesToUserMessages = resolveConvertUnsupportedSystemMessagesToUserMessagesOption(
    ...
    if (
      this.convertUnsupportedSystemMessagesToUserMessages != null &&
      !this.convertUnsupportedSystemMessagesToUserMessages.format.includes('{{message}}')
    )
  ...
  public override generateContextState({
    ...
    if (this.convertUnsupportedSystemMessagesToUserMessages.use === 'always') {
      return this._generateContextText(historyWithFunctions, {
  ...
  private _generateContextText(
    ...
    const transformedHistory =
      convertSystemMessagesToUserMessagesFormat == null
        ? history
        : history.map((item) => {
            if (item.type === 'system') {
    ...
    for (const item of transformedHistory) {
      if (item.type === 'system') {
        if (!this.joinAdjacentMessagesOfTheSameType || currentAggregateFocus !== 'system') flush();

        currentAggregateFocus = 'system';
        currentTexts.push(LlamaText.fromJSON(item.text));
      } else if (item.type === 'user') {
        if (!this.joinAdjacentMessagesOfTheSameType || currentAggregateFocus !== 'user') flush();

        currentAggregateFocus = 'user';
        currentTexts.push(LlamaText(item.text));
      } else if (item.type === 'model') {
        if (!this.joinAdjacentMessagesOfTheSameType || currentAggregateFocus !== 'model') flush();
    ...
    const idsGenerator = new UniqueTemplateId(
      this.template +
        ...
        this.systemRoleName +
        (convertSystemMessagesToUserMessagesFormat ?? '') +
        resultItems.map(({ content }) => content.toString()).join('\n\n'),
    );
    ...
    for (const resultItem of resultItems) {
      ...
      if (resultItem.role === 'model') modelMessageIds.add(id);
    }
    ...
    function tryOptions<const T extends (() => any)[]>(options: T): ReturnType<T[number]> {
      for (let i = 0; i < options.length; i++) {
        if (i === options.length - 1) return options[i]!();
      ...
      return tryOptions([
        () =>
          this._jinjaTemplate.render({
            ...(this.additionalRenderParameters == null ? {} : structuredClone(this.additionalRenderParameters)),
            messages: jinjaItems,
    ...
    const validateThatAllMessageIdsAreUsed = (parts: ReturnType<typeof splitText<string[]>>) => {
      ...
      if (messageIdsLeft.size !== 0) throw new Error('Some input messages are not present in the generated Jinja template output');
    };
    ...
    const renderJinjaAndSplitIntoParts = () => {
      ...
      if (lastItemIsModelMessage) {
        ...
        for (let i = splitJinjaParts.length - 1; i >= 0; i--) {
          ...
          if (messageIds.has(part.separator)) {
    ...
    const contextText = LlamaText(
      splitJinjaParts.map((part) => {
        if (typeof part === 'string') return new SpecialTokensText(part); // things that are not message content can be tokenized with special tokens

        const message = idToContent.get(part.separator);

        if (message == null) throw new Error(`Message with id "${part.separator}" not found`);
    ...
    if (messageIdsLeftToProcess.size !== 0) throw new Error('Some input messages are not present in the generated Jinja template output');
          ...
          : [
              LlamaText(
                stopGenerationJinjaParts.map((part) => {
                  if (typeof part === 'string') return new SpecialTokensText(part);
  ...
  private _runSanityTest() {
    try {
      ...
      for (const chatHistory of chatHistoriesForSanityTest) {
        ...
        if (transformedSystemMessagesToUserMessages) supportsSystemMessages = false;
      }

      return { supportsSystemMessages };
    } catch (err) {
      throw new Error(
        `The provided Jinja template failed the sanity test: ${String(err)}. Inspect the Jinja template to find out what went wrong`,
      );
    }
  }
}
...
class UniqueTemplateId {
  ...
  public generateId(): string {
    ...
    do {
      id = `W${Math.random().toString(36).slice(2)}W`;
...
function resolveConvertUnsupportedSystemMessagesToUserMessagesOption(
  ...
  if (convertUnsupportedSystemMessagesToUserMessages === false) return undefined;
