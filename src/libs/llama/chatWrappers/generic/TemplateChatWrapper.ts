...
import type { ChatWrapperGenerateContextStateOptions, ChatWrapperGeneratedContextState, ChatWrapperSettings } from '../../types';
import { LlamaText, type LlamaTextValue, SpecialToken, SpecialTokensText } from '../../utils/LlamaText.js';
import { parseTextTemplate } from '../../utils/parseTextTemplate';
import {
  ...
  public override generateContextState({
    ...
    for (const item of historyWithFunctions) {
      if (item.type === 'system') {
        ...
        if (!this.joinAdjacentMessagesOfTheSameType || (currentAggregateFocus !== 'system' && currentAggregateFocus !== 'user')) flush();

        currentAggregateFocus = 'user';
        userTexts.push(LlamaText(item.text));
      } else if (item.type === 'model') {
        if (!this.joinAdjacentMessagesOfTheSameType) flush();
    ...
    const getHistoryItem = (role: 'system' | 'user' | 'model', text: LlamaText, prefix?: string | null) => {
      ...
      return LlamaText([new SpecialTokensText((prefix ?? '') + messagePrefix), text, new SpecialTokensText(messageSuffix)]);
    };
    ...
    const contextText = LlamaText(
      resultItems.map(({ system, user, model }, index) => {
        ...
        const res = LlamaText([
          ...
          user.values.length === 0 ? LlamaText([]) : getHistoryItem('user', user),

          model.values.length === 0
            ? LlamaText([])
            : !isLastItem
              ? getHistoryItem('model', model)
              : LlamaText([new SpecialTokensText(this._parsedChatTemplate.completionPrefix), model]),
        ]);
        ...
        return LlamaText(
          res.values.reduce((res, value) => {
            if (value instanceof SpecialTokensText) {
              ...
              if (lastItem == null || !(lastItem instanceof SpecialTokensText)) return res.concat([value]);

              return res.slice(0, -1).concat([new SpecialTokensText(lastItem.value + value.value)]);
            }
...
function parseChatTemplate(template: TemplateChatWrapperOptions['template']): {
  ...
  if (parsedTemplate.completion.suffix.length === 0) throw new Error('Chat template must have text after "{{completion}}"');
