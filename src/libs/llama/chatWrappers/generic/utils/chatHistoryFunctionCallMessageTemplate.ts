import type { ChatWrapperSettings } from '../../../types.js';
...
export function parseFunctionCallMessageTemplate(
  ...
  if (callPrefix.length === 0) throw new Error('Function call template must have text before "{{functionName}}"');

  if (callSuffix.length === 0) throw new Error('Function call template must have text after "{{functionParams}}"');

  if (resultPrefix.length === 0) throw new Error('Function call result template must have text before "{{functionCallResult}}"');

  if (resultSuffix.length === 0) throw new Error('Function call result template must have text after "{{functionCallResult}}"');
