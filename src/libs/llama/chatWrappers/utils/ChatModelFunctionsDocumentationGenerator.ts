import type { ChatModelFunctions } from '../../types';
...
export class ChatModelFunctionsDocumentationGenerator {
  ...
  public getTypeScriptFunctionSignatures({ documentParams = true }: { documentParams?: boolean } = {}) {
    ...
    if (!this.hasAnyFunctions || chatModelFunctions == null) return '';
    ...
    return functionNames
      .map((functionName) => {
        ...
        else if (!documentParams && functionDefinition?.params != null) res += 'params';
   ...
   */
  public getTypeScriptFunctionTypes({
    documentParams = true,
    ...
    return functionNames
      .map((functionName) => {
        if (reservedFunctionNamesSet.has(functionName)) throw new Error(`Function name "${functionName}" is reserved and cannot be used`);
  ...
  public getLlama3_1FunctionSignatures({ documentParams = true }: { documentParams?: boolean } = {}) {
    ...
    return functionNames
      .map((functionName) => {
        ...
        if (addDescription) res += ` to: ${functionDefinition.description.split('\n').join('\n// ')}\n`;
        else res += '.\n';
