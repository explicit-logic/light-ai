import type { ChatHistoryItem, ChatModelResponse } from '../../types.js';
import { LlamaText } from '../../utils/LlamaText.js';

export function chunkChatItems(
  ...
  for (const item of chatHistory) {
    if (item.type === 'system') {
      if (!joinAdjacentMessagesOfTheSameType || currentAggregateFocus !== 'system') flush();

      currentAggregateFocus = 'system';
      systemTexts.push(LlamaText.fromJSON(item.text));
    } else if (item.type === 'user') {
      if (!joinAdjacentMessagesOfTheSameType || (currentAggregateFocus !== 'system' && currentAggregateFocus !== 'user')) flush();

      currentAggregateFocus = 'user';
      userTexts.push(LlamaText(item.text));
    } else if (item.type === 'model') {
      if (!joinAdjacentMessagesOfTheSameType) flush();
