...
import type { ChatHistoryItem, ChatModelResponse, ChatUserMessage, Tokenizer } from '../../types.js';
...
import { JinjaTemplateChatWrapper, type JinjaTemplateChatWrapperOptions } from '../generic/JinjaTemplateChatWrapper.js';
...
export function isJinjaTemplateEquivalentToSpecializedChatWrapper(
  ...
  try {
    ...
    if (checkEquivalence(jinjaChatWrapper, specializedChatWrapper, testChatHistories, tokenizer)) return true;
  } catch (err) {
    // Do nothing
  }
  ...
  try {
    ...
    if (checkEquivalence(jinjaChatWrapperWithLeadingWhitespaceTrimming, specializedChatWrapper, testChatHistories, tokenizer)) return true;
  } catch (err) {
    // Do nothing
  }

  if (!canTestMultipleConvertSystemMessagesToUserMessages) return false;

  const convertSystemMessagesToUserMessagesTemplate = '### System message\n\n{{message}}\n\n----';
  const transformedTestChatHistories = testChatHistories.map((history) =>
    history
      .slice()
  ...
  try {
    ...
    if (checkEquivalence(jinjaChatWrapper, specializedChatWrapper, transformedTestChatHistories, tokenizer)) return true;
  } catch (err) {
    // Do nothing
  }
...
function checkEquivalence(
  ...
  for (const testChatHistory of testChatHistories) {
    ...
    if (!compareContextTexts(jinjaRes.contextText, specializedWrapperRes.contextText, tokenizer)) return false;

    const jinjaHasAllSpecializedStopGenerationTriggers = jinjaRes.stopGenerationTriggers.every((trigger) => {
      return [trigger, trigger.trimEnd(), trigger.trimStart(), trigger.trimStart().trimEnd()].some((normalizedJinjaTrigger) => {
        if (normalizedJinjaTrigger.values.length === 0) return true;
        ...
        if (foundSimilarTriggers) return true;

        if (tokenizer != null) {
          const resolvedStopGenerationTrigger = StopGenerationDetector.resolveLlamaTextTrigger(normalizedJinjaTrigger, tokenizer);

          const foundSimilarOrShorterTokenizedTriggers = specializedWrapperRes.stopGenerationTriggers.some((specializedTrigger) => {
            ...
            return resolvedSpecializedTrigger.every((item, index) => {
              ...
              if (typeof item === 'string' && typeof resolveTriggerItem === 'string') return item === resolveTriggerItem;
              if (typeof item === 'string' || typeof resolveTriggerItem === 'string' || resolveTriggerItem == null) return false;

              return compareTokens(item, resolveTriggerItem);
            });
          });

          if (foundSimilarOrShorterTokenizedTriggers) return true;
        }
    ...
    if (!jinjaHasAllSpecializedStopGenerationTriggers) return false;
  }
...
function compareContextTexts(text1: LlamaText, text2: LlamaText, tokenizer?: Tokenizer) {
  function compare(text1: LlamaText, text2: LlamaText) {
    if (LlamaText.compare(text1, text2)) return true;
  ...
  const texts1 =
    normalizedText1.values.length !== trimmedText1.values.length && tokenizer != null ? [trimmedText1, normalizedText1] : [normalizedText1];

  const texts2 =
    normalizedText2.values.length !== trimmedText2.values.length && tokenizer != null ? [trimmedText2, normalizedText2] : [normalizedText2];
...
function removeLeadingBos(llamaText: LlamaText) {
  if (llamaText.values.length === 0) return llamaText;

  const firstValue = llamaText.values[0];

  if (firstValue instanceof SpecialToken && firstValue.value === 'BOS') return LlamaText(llamaText.values.slice(1));
