...
import { includesText } from '../../utils/includesText';
import { parseModelFileName } from '../../utils/parseModelFileName';
...
import { JinjaTemplateChatWrapper, type JinjaTemplateChatWrapperOptions } from '../generic/JinjaTemplateChatWrapper';
...
export const templateChatWrapperTypeNames = Object.freeze(['template', 'jinjaTemplate'] as const);
...
const chatWrapperToConfigType = new Map(
  Object.entries(chatWrappers).map(([configType, Wrapper]) => [Wrapper, configType as keyof typeof chatWrappers]),
);
...
export function resolveChatWrapper(options: ResolveChatWrapperOptions): BuiltInChatWrapperType | null {
    ...
    if (isTemplateChatWrapperType(type)) {
      ...
      if (isClassReference(Wrapper, TemplateChatWrapper)) {
        ...
        if (
          ...
          wrapperSettings?.template == null ||
          wrapperSettings?.historyTemplate == null ||
          wrapperSettings.historyTemplate.system == null ||
          wrapperSettings.historyTemplate.user == null ||
          wrapperSettings.historyTemplate.model == null
        ) {
          if (warningLogs)
            console.warn(
              `${getConsoleLogPrefix()}Template chat wrapper settings must have a template, historyTemplate, historyTemplate.system, historyTemplate.user, and historyTemplate.model. Falling back to resolve other chat wrapper types.`,
            );
        } else return new TemplateChatWrapper(wrapperSettings);
        ...
        if (jinjaTemplate == null) {
          if (warningLogs)
            console.warn(
              `${getConsoleLogPrefix()}Jinja template chat wrapper received no template. Falling back to resolve other chat wrapper types.`,
            );
        } else {
          try {
            ...
            if (!fallbackToOtherWrappersOnJinjaError) throw err;
            if (warningLogs)
              console.error(
                `${getConsoleLogPrefix()}Error creating Jinja template chat wrapper. Falling back to resolve other chat wrappers. Error:`,
                err,
              );
          }
        }
      } else void (Wrapper satisfies never);
    } else if (Object.hasOwn(chatWrappers, type)) {
      const Wrapper = chatWrappers[type];
      const wrapperSettings: ConstructorParameters<typeof Wrapper>[0] | undefined = customWrapperSettings?.[type];
  ...
  if (modelJinjaTemplate != null && modelJinjaTemplate.trim() !== '') {
    ...
    const chatWrapperNamesToCheck = orderChatWrapperNamesByAssumedCompatibilityWithModel(specializedChatWrapperTypeNames, {
      filename,
      fileInfo,
    });
    for (const specializedChatWrapperTypeName of chatWrapperNamesToCheck) {
      ...
      if (testOptionConfigurations.length === 0) testOptionConfigurations.push({} as any);

      for (const testConfigurationOrPair of testOptionConfigurations) {
        const testConfig = Array.isArray(testConfigurationOrPair) ? (testConfigurationOrPair[0]! ?? {}) : testConfigurationOrPair;
        const applyConfig = Array.isArray(testConfigurationOrPair) ? (testConfigurationOrPair[1]! ?? {}) : testConfigurationOrPair;
        const additionalJinjaParameters = Array.isArray(testConfigurationOrPair) ? testConfigurationOrPair[2]! : undefined;
        ...
        const jinjaTemplateChatWrapperOptionsWithAdditionalParameters: JinjaTemplateChatWrapperOptions = {
          ...
          additionalRenderParameters:
            additionalJinjaParameters == null
        ...
        if (
          isJinjaTemplateEquivalentToSpecializedChatWrapper(jinjaTemplateChatWrapperOptionsWithAdditionalParameters, chatWrapper, tokenizer)
        )
    ...
    if (!noJinja) {
      if (!fallbackToOtherWrappersOnJinjaError) return new JinjaTemplateChatWrapper(jinjaTemplateChatWrapperOptions);
  ...
  for (const modelNames of getModelLinageNames(fileInfo?.metadata)) {
    if (
      includesText(modelNames, ['llama 3.2', 'llama-3.2', 'llama3.2']) &&
      Llama3_2LightweightChatWrapper._checkModelCompatibility({ tokenizer, fileInfo })
    )
      return createSpecializedChatWrapper(Llama3_2LightweightChatWrapper);
    if (
      includesText(modelNames, ['llama 3.1', 'llama-3.1', 'llama3.1']) &&
      Llama3_1ChatWrapper._checkModelCompatibility({ tokenizer, fileInfo })
    )
      return createSpecializedChatWrapper(Llama3_1ChatWrapper);
    if (includesText(modelNames, ['llama 3', 'llama-3', 'llama3'])) return createSpecializedChatWrapper(Llama3ChatWrapper);
    if (includesText(modelNames, ['Mistral', 'Mistral Large', 'Mistral Large Instruct', 'Mistral-Large', 'Codestral']))
      return createSpecializedChatWrapper(MistralChatWrapper);
    if (includesText(modelNames, ['Gemma', 'Gemma 2'])) return createSpecializedChatWrapper(GemmaChatWrapper);
  }
  ...
  if (modelJinjaTemplate != null && modelJinjaTemplate.trim() !== '') {
    if (modelJinjaTemplate.includes('<|im_start|>')) return createSpecializedChatWrapper(ChatMLChatWrapper);
    if (modelJinjaTemplate.includes('[INST]'))
      return createSpecializedChatWrapper(Llama2ChatWrapper, {
        addSpaceBeforeEos: modelJinjaTemplate.includes("' ' + eos_token"),
      });
    if (modelJinjaTemplate.includes('<|start_header_id|>') && modelJinjaTemplate.includes('<|end_header_id|>')) {
      if (Llama3_1ChatWrapper._checkModelCompatibility({ tokenizer, fileInfo })) return createSpecializedChatWrapper(Llama3_1ChatWrapper);
      return createSpecializedChatWrapper(Llama3ChatWrapper);
    }
    if (modelJinjaTemplate.includes('<start_of_turn>')) return createSpecializedChatWrapper(GemmaChatWrapper);
  }
  ...
  if (filename != null) {
    ...
    if (fileType?.toLowerCase() === 'gguf') {
      ...
      if (lowercaseName === 'llama') {
        if (splitLowercaseSubType.includes('chat')) return createSpecializedChatWrapper(Llama2ChatWrapper);

        return createSpecializedChatWrapper(GeneralChatWrapper);
      }
      if (lowercaseName === 'codellama') return createSpecializedChatWrapper(GeneralChatWrapper);
      if (lowercaseName === 'yarn' && firstSplitLowercaseSubType === 'llama') return createSpecializedChatWrapper(Llama2ChatWrapper);
      if (lowercaseName === 'orca') return createSpecializedChatWrapper(ChatMLChatWrapper);
      if (lowercaseName === 'phind' && lowercaseSubType === 'codellama') return createSpecializedChatWrapper(Llama2ChatWrapper);
      if (lowercaseName === 'mistral') return createSpecializedChatWrapper(GeneralChatWrapper);
      if (firstSplitLowercaseSubType === 'llama') return createSpecializedChatWrapper(Llama2ChatWrapper);
      if (lowercaseSubType === 'alpaca') return createSpecializedChatWrapper(AlpacaChatWrapper);
      if (lowercaseName === 'functionary') return createSpecializedChatWrapper(FunctionaryChatWrapper);
      if (lowercaseName === 'dolphin' && splitLowercaseSubType.includes('mistral')) return createSpecializedChatWrapper(ChatMLChatWrapper);
      if (lowercaseName === 'gemma') return createSpecializedChatWrapper(GemmaChatWrapper);
      if (splitLowercaseSubType.includes('chatml')) return createSpecializedChatWrapper(ChatMLChatWrapper);
    }
  }
  ...
  if (fileInfo != null) {
    ...
    if (arch === 'llama') return createSpecializedChatWrapper(GeneralChatWrapper);
    if (arch === 'falcon') return createSpecializedChatWrapper(FalconChatWrapper);
    if (arch === 'gemma' || arch === 'gemma2') return createSpecializedChatWrapper(GemmaChatWrapper);
  }
...
function orderChatWrapperNamesByAssumedCompatibilityWithModel<T extends ResolvableChatWrapperTypeName>(
  chatWrapperNames: readonly T[],
  {
    filename,
    fileInfo,
  }: {
  ...
  function getPointsForTextMatch(pattern: string, fullText: string | undefined, existsPoints: number, positionPoints: number) {
    ...
    if (index >= 0) return existsPoints + ((index + 1) / fullText.length) * positionPoints;
  ...
  return chatWrapperNames.slice().sort((a, b) => {
    let aPoints = 0;
    let bPoints = 0;
