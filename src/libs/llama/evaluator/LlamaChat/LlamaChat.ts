...
import { UNKNOWN_UNICODE_CHAR, maxRecentDetokenizerTokens } from '../../consts.js';
import type {
  ChatHistoryItem,
  ChatModelFunctions,
  ChatModelResponse,
...
import { LlamaText, type LlamaTextJSON, SpecialToken } from '../../utils/LlamaText';
import { StopGenerationDetector } from '../../utils/StopGenerationDetector';
import { type QueuedTokenRelease, type QueuedTokenReleaseLock, TokenStreamRegulator } from '../../utils/TokenStreamRegulator.js';
import type { GbnfJsonSchemaToType } from '../../utils/gbnfJson/types';
...
import { resolveLastTokens } from '../../utils/resolveLastTokens';
...
import type { EvaluationPriority } from '../LlamaContext/types';
...
export class LlamaChat {
  ...
  public constructor({ contextSequence, chatWrapper = 'auto', autoDisposeSequence = false }: LlamaChatOptions) {
    if (contextSequence == null) throw new Error('contextSequence cannot be null');

    if (contextSequence.disposed) throw new DisposedError();
  ...
  public dispose({ disposeSequence = this._autoDisposeSequence }: { disposeSequence?: boolean } = {}) {
    if (this._sequence == null) return;

    if (disposeSequence) this._sequence.dispose();
  ...
  public get chatWrapper() {
    if (this._sequence == null) throw new DisposedError();
  ...
  public async generateResponse<const Functions extends ChatModelFunctions | undefined = undefined>(
    ...
    const generateResponseState = new GenerateResponseState<Functions>(this, this._chatWrapper, history, {
      onTextChunk,
      onToken,
      signal,
      stopOnAbortSignal,
      ...
      try {
        ...
        while (true) {
          ...
          if (generateResponseState.functionEvaluationMode !== false) {
            ...
            if (functionsCallsRes != null) return functionsCallsRes;
          ...
          while (await generateResponseState.iterateEvaluation()) {
            ...
            if (stopGenerationTriggerRes != null) return stopGenerationTriggerRes;
            ...
            if (maxTokensTriggerRes != null) return maxTokensTriggerRes;

            if (generateResponseState.updateShouldContextShift()) break;

            const abortRes = generateResponseState.handleAbortTrigger('model');
            if (abortRes != null) return abortRes;
          }

          generateResponseState.isFirstEvaluation = false;

          if (generateResponseState.shouldContextShift) continue;
  ...
  public async loadChatAndCompleteUserMessage<const Functions extends ChatModelFunctions | undefined = undefined>(
    ...
    const lastEvaluationContextWindowHistoryItem =
      lastEvaluationContextWindowHistory == null ? null : lastEvaluationContextWindowHistory[lastEvaluationContextWindowHistory.length - 1];
    const lastEvaluationContextWindowUserMessage =
      lastEvaluationContextWindowHistoryItem?.type === 'user' ? lastEvaluationContextWindowHistoryItem.text : '';
    ...
    const generateResponseState = new GenerateResponseState<Functions>(this, this._chatWrapper, history, {
      ...
      lastEvaluationContextWindow: {
        history:
          lastEvaluationContextWindowHistory == null
            ? undefined
    ...
    return await withLock(this._chatLock, 'evaluate', signal, async (): Promise<LlamaChatLoadAndCompleteUserResponse> => {
      try {
        ...
        const getInitialUserMessage = (history: ChatHistoryItem[]) => {
          ...
          if (lastResolvedHistoryItem?.type === 'user') return lastResolvedHistoryItem.text;
        ...
        while (true) {
          ...
          const { userTextSuffix } = await generateResponseState.loadContextWindow(
            ...
            setLastUserTextInChatHistory(
              ...
              contextWindowInitialUserMessage + initialUserPrompt + this.model.detokenize(generateResponseState.contextWindowsRes),
            ),
            true,
          );
          ...
          while (await generateResponseState.iterateEvaluation()) {
            ...
            if (stopGenerationTriggerRes != null)
              return {
                ...
                metadata:
                  stopGenerationTriggerRes.metadata.stopReason === 'customStopTrigger'
...
export type LlamaChatResponseFunctionCall<
  ...
  Params = Functions[FunctionCallName]['params'] extends undefined | null | undefined
    ? undefined
...
async function compressHistoryToFitContextSize({
  ...
  if (contextShiftStrategy instanceof Function) {
    try {
      ...
      console.warn(
        'The provided context shift strategy did not return a history that fits the context size. ' + 'Using the default strategy instead.',
      );
    } catch (err) {
      console.error('The provided context shift strategy threw an error. ' + 'Using the default strategy instead.', err);
    }
  } else if (contextShiftStrategy !== 'eraseFirstResponseAndKeepFirstSystem')
    console.warn(`Unknown context shift strategy "${contextShiftStrategy}". Using the default strategy instead.`);
...
function getLastTextModelResponseFromChatHistory(chatHistory: ChatHistoryItem[]) {
  if (chatHistory.length === 0 || chatHistory[chatHistory.length - 1]!.type !== 'model') return '';
...
function getLastUserTextFromChatHistory(chatHistory: readonly ChatHistoryItem[]) {
  if (chatHistory.length === 0 || chatHistory[chatHistory.length - 1]!.type !== 'user') return '';
...
function setLastModelTextResponseInChatHistory(chatHistory: ChatHistoryItem[], textResponse: string) {
  ...
  if (modelResponse.length > 0 && typeof modelResponse[modelResponse.length - 1] === 'string') {
    if (textResponse === '') modelResponse.pop();
    else modelResponse[modelResponse.length - 1] = textResponse;
  } else if (textResponse !== '') modelResponse.push(textResponse);
...
function setLastTextInChatHistory(itemType: 'user' | 'model', chatHistory: ChatHistoryItem[], text: string) {
  if (itemType === 'user') return setLastUserTextInChatHistory(chatHistory, text);
...
function generateContextText(
  ...
  if (endWithUserText) return generateContextTextThatEndsWithUserText(chatWrapper, options);
...
function generateContextTextThatEndsWithUserText(
  ...
  const randomId = `W${Math.random().toString(36).slice(2)}W`;
  ...
  for (let i = 0; i < newContextText.values.length; i++) {
    ...
    if (randomTextIndex < 0) continue;

    const newValue = item.slice(0, randomTextIndex);
    newContextText = LlamaText([...newContextText.values.slice(0, i), newValue]);
    return {
      contextText: newContextText,
      userTextSuffix: LlamaText([item.slice(randomTextIndex + randomId.length), ...newContextText.values.slice(i + 1)]),
      ...rest,
    };
  }

  throw new Error(
    `The random ID was not found in the context text. There might be an issue with the chat wrapper "${chatWrapper.wrapperName}" where not all user messages are properly added to the the result LlamaText`,
  );
}
...
async function getContextWindow({
  ...
  chatWrapper: ChatWrapper;
  lastEvaluationContextWindowHistory?: ChatHistoryItem[];
  minimumOverlapPercentageToPreventContextShift: number;
  ...
  tokens: Token[];
  newResolvedHistory: ChatHistoryItem[];
  newHistoryCompressionMetadata: object | null | undefined;
  ignoreStartText: LlamaText[];
  functionCallInitiallyEngaged: boolean;
  disengageInitiallyEngagedFunctionCall: LlamaText[];
  userTextSuffix?: LlamaText;
}> {
  if (sequence == null) throw new DisposedError();
  ...
  resolvedHistory = sequence.isLoadedToMemory ? resolvedHistory.slice() : resolvedHistory.map(removeRawFromHistoryItem);

  if (resolvedContextShift.lastEvaluationMetadata != null) {
    const contextShiftSize =
      resolvedContextShift.size instanceof Function ? await resolvedContextShift.size(sequence) : resolvedContextShift.size;

    const { compressedHistory, metadata } = await compressHistoryToFitContextSize({
      history: resolvedHistory,
      contextShiftSize:
        Math.max(minFreeContextTokens, Math.min(contextShiftSize, context.contextSize - pendingTokensCount)) + pendingTokensCount,
  ...
  const contextShiftSize = Math.min(
    ...
    Math.max(
      1,
      Math.floor(resolvedContextShift.size instanceof Function ? await resolvedContextShift.size(sequence) : resolvedContextShift.size),
    ),
  );
...
class GenerateResponseState<const Functions extends ChatModelFunctions | undefined = undefined> {
  ...
  private readonly minimumOverlapPercentageToPreventContextShift: Exclude<
    Exclude<
      LLamaChatGenerateResponseOptions<Functions>['lastEvaluationContextWindow'],
      undefined
    >['minimumOverlapPercentageToPreventContextShift'],
    undefined
  >;
  ...
  private functionEvaluationFunctionName = '';
  private currentFunctionCallPreviousPartLeftoverText = '';
  private removedStartTextToIgnore = false;
  private releasedPartiallyFreeTokensBeforeFunctionCallStartSyntax = false;
  ...
  public constructor(
    ...
    if (this.llamaChat.disposed) throw new DisposedError();

    this.resolvedHistory = this.llamaChat.sequence.isLoadedToMemory ? this.history.slice() : this.history.map(removeRawFromHistoryItem);
    this.resolvedContextShift = {
      ...defaultContextShiftOptions,
      ...removeNullFields(this.contextShift),
    };
    this.resolvedRepeatPenalty =
      repeatPenalty === false
        ? { lastTokens: 0 }
        : {
            ...(repeatPenalty ?? {}),
    ...
    this.grammarEvaluationState =
      this.grammar != null ? new LlamaGrammarEvaluationState({ model: this.llamaChat.model, grammar: this.grammar }) : undefined;
    ...
    if (this.customStopTriggers != null)
      StopGenerationDetector.resolveStopTriggers(this.customStopTriggers, this.llamaChat.model.tokenizer).map((stopTrigger) =>
        this.customStopGenerationTriggersDetector.addStopTrigger(stopTrigger),
      );

    if (this.grammar != null)
      StopGenerationDetector.resolveStopTriggers(this.grammar.stopGenerationTriggers, this.llamaChat.model.tokenizer).map((stopTrigger) =>
        this.stopGenerationDetector.addStopTrigger(stopTrigger),
      );
  ...
  public ensureNotAborted() {
    if (this.signal?.aborted && (!this.stopOnAbortSignal || this.res.length === 0)) throw this.signal.reason;
  ...
  public getPenaltyTokens() {
    ...
    if (this.resolvedRepeatPenalty.punishTokensFilter != null) punishTokens = this.resolvedRepeatPenalty.punishTokensFilter(punishTokens);
  ...
  public getResolvedHistoryWithCurrentModelResponse() {
    if (this.res.length === 0) return this.resolvedHistory;

    let modelResponse = this.llamaChat.model.detokenize(this.res);

    if (this.grammar?.trimWhitespaceSuffix || this.trimWhitespaceSuffix) modelResponse = modelResponse.trimEnd();

    if (modelResponse === '') return this.resolvedHistory;
  ...
  public getContextWindowsHistoryWithCurrentModelResponse() {
    if (this.contextWindowsRes.length === 0) return this.lastContextWindowHistory;

    let modelResponse = this.llamaChat.model.detokenize(this.contextWindowsRes);

    if (this.grammar?.trimWhitespaceSuffix || this.trimWhitespaceSuffix) modelResponse = modelResponse.trimEnd();

    if (modelResponse === '') return this.lastContextWindowHistory;
  ...
  public removeFoundStartIgnoreTextsFromPendingTokens(forceRemove = false) {
    if (
      !this.removedStartTextToIgnore &&
      this.res.length === 0 &&
      this.pendingTokens.length > 0 &&
      this.ignoreStartTextDetector.hasTriggeredStops &&
      (forceRemove || !this.ignoreStartTextDetector.hasInProgressStops)
    ) {
      ...
      const lastTokensForDetokenizer = resolveLastTokens([this.contextWindowTokens, this.ignoredStartTextTokens]);
      for (let i = 0; i < this.pendingTokens.length; i++) {
        ...
        } else if (!this.ignoreStartTextDetector.hasInProgressStops) break;
      }
      ...
      if (mostExhaustiveTriggeredStops != null) {
        ...
        if (mostExhaustiveTriggeredStop != null) {
          this.ignoredStartTextTokens = mostExhaustiveTriggeredStop.stopTrigger.flatMap((stopTrigger) => {
            if (typeof stopTrigger === 'string') return this.llamaChat.model.tokenize(stopTrigger, false, 'trimLeadingSpace');
            return [stopTrigger];
          });

          const newPendingTokens = [...mostExhaustiveTriggeredStop.remainingGeneration, mostExhaustiveTriggeredStopsLeftoverTokens].flatMap(
            (generation) => {
              if (typeof generation === 'string') return this.llamaChat.model.tokenize(generation, false, 'trimLeadingSpace');
              return generation;
            },
          );
          this.pendingTokens.length = 0;
  ...
  private getContextWindowFunctionCallsTokens() {
    if (this.functionEvaluationMode === false) return [];
    ...
    for (let i = 0; i < this.resFunctionCalls.length; i++) {
      ...
      if (i > 0) text.push(this.chatWrapper.settings.functions?.parallelism?.call?.betweenCalls ?? '');
    ...
    return [...LlamaText(text).tokenize(this.llamaChat.model.tokenizer, 'trimLeadingSpace'), ...this.currentFunctionCallCurrentPartTokens];
  }

  public async loadContextWindow(
    resolvedHistory: ChatHistoryItem[],
    resolvedContextWindowsHistory: ChatHistoryItem[],
    endWithUserText = false,
    avoidReloadingHistory = false,
  ...
  public addIgnoreStartTextTriggersFromChatWrapper() {
    StopGenerationDetector.resolveStopTriggers(this.ignoreStartText, this.llamaChat.model.tokenizer).map((stopTrigger) =>
      this.ignoreStartTextDetector.addStopTrigger(stopTrigger),
    );
  }

  public addStopGenerationTriggersFromChatWrapper() {
    StopGenerationDetector.resolveStopTriggers(this.stopGenerationTriggers, this.llamaChat.model.tokenizer).map((stopTrigger) =>
      this.stopGenerationDetector.addStopTrigger(stopTrigger),
    );
  }
  ...
  public initFunctions() {
    ...
    if (this.initiallyEngagedFunctionMode) {
      StopGenerationDetector.resolveStopTriggers(this.disengageInitiallyEngagedFunctionCall, this.llamaChat.model.tokenizer).map(
        (stopTrigger) => this.disengageInitiallyEngagedFunctionMode.addStopTrigger(stopTrigger),
      );
  ...
  public async enterFunctionCallingLoop(loadContextWindow: () => Promise<void>) {
    ...
    while (true) {
      if (this.functionEvaluationMode === 'prefixOrDisengage') {
        ...
        const prefixTokens = LlamaText(this.chatWrapper.settings.functions.call.prefix).tokenize(
          this.llamaChat.model.tokenizer,
          'trimLeadingSpace',
        );
        ...
        for (const prefixToken of prefixTokens) {
          ...
          const disregardedPossibilities = this.disengageInitiallyEngagedFunctionMode.getDisregardedPossibilitiesCountForAGeneration({
            text,
            tokens,
            startNewChecks: this.currentFunctionCallCurrentPartTokens.length === 0,
          });

          if (disregardedPossibilities > 0) break;
          ...
          if (prefixDetector.hasTriggeredStops) afterPrefixLeftoverTokens.push(prefixToken);
          else {
        ...
        for await (const token of this.evaluateWithContextShift(loadContextWindow)) {
          ...
          if (prefixDetector.hasTriggeredStops) afterPrefixLeftoverTokens.push(token);
          else {
            prefixDetector.recordGeneration({
              text: this.currentText,
              tokens: this.currentTokens,
          ...
          if (
            this.disengageInitiallyEngagedFunctionMode.hasTriggeredStops ||
        ...
        if (prefixDetector.hasTriggeredStops) {
          ...
          const { firstRemainingGenerationAfterStop, stopTrigger } =
          ...
          this.currentFunctionCallPreviousPartLeftoverText =
            StopGenerationDetector.detokenizeRemainingGeneration(
        ...
        } else this.currentFunctionCallPreviousPartLeftoverText = '';
      ...
      if (this.functionEvaluationMode === 'functionName') {
        ...
        this.currentFunctionCallCurrentPartTokens.length = 0;
        const functionNameGrammar =
          this.functionNameGrammar ??
          new FunctionCallNameGrammar(this.llamaChat.model._llama, this.functions as NonNullable<Functions>, this.chatWrapper);
        ...
        StopGenerationDetector.resolveStopTriggers(this.functionsGrammar.stopGenerationTriggers, this.llamaChat.model.tokenizer).map(
          (stopTrigger) => functionNameGenerationDoneDetector.addStopTrigger(stopTrigger),
        );
        ...
        if (this.currentFunctionCallPreviousPartLeftoverText !== '') {
          ...
          const hasAnyFunctionStartWithLeftover = validFunctionNames.some((functionName) =>
            functionName.startsWith(this.currentFunctionCallPreviousPartLeftoverText),
          );

          if (hasAnyFunctionStartWithLeftover) {
            const leftoverTokens = this.llamaChat.model.tokenize(
              this.currentFunctionCallPreviousPartLeftoverText,
              false,
              'trimLeadingSpace',
            );
            ...
            for (const leftoverToken of leftoverTokens) {
              const canBeNextToken = LlamaSampler._canBeNextTokenForGrammarEvaluationState(
                this.llamaChat.model._llama,
                this.functionsEvaluationState,
                leftoverToken,
              );

              if (!canBeNextToken) break;
        ...
        for await (const token of this.evaluateWithContextShift(loadContextWindow)) {
          ...
          if (functionNameGenerationDoneDetector.hasTriggeredStops) break;
        }
      ...
      if (this.functionEvaluationMode === 'params') {
        this.currentFunctionCallPreviousText = LlamaText([
          this.chatWrapper.settings.functions.call.prefix,
        ...
        if (functionDefinition.params == null) {
          params = undefined;
          paramsText = '';
        } else {
          ...
          StopGenerationDetector.resolveStopTriggers(this.functionsGrammar.stopGenerationTriggers, this.llamaChat.model.tokenizer).map(
            (stopTrigger) => functionParamsGenerationDoneDetector.addStopTrigger(stopTrigger),
          );
          ...
          for await (const token of this.evaluateWithContextShift(loadContextWindow)) {
            ...
            if (functionParamsGenerationDoneDetector.hasTriggeredStops) break;
          }
        ...
        if (
          this.chatWrapper.settings.functions.parallelism == null ||
      ...
      if (this.functionEvaluationMode === 'sectionSuffixOrBetweenCalls') {
        ...
        for await (const token of this.evaluateWithContextShift(loadContextWindow)) {
          ...
          if (sectionSuffixDetector.hasTriggeredStops || !sectionSuffixDetector.hasInProgressStops) break;
        }
  ...
  public releasePartiallyFreeTokensBeforeFunctionCallStart() {
    if (this.releasedPartiallyFreeTokensBeforeFunctionCallStartSyntax) return;
  ...
  public async *evaluateWithContextShift(loadContextWindow: () => Promise<void>): AsyncGenerator<Token> {
    while (true) {
      ...
      while (await this.iterateEvaluation()) {
        if (this.currentToken == null) break;

        yield this.currentToken;

        if (this.shouldAbort) return;

        if (this.updateShouldContextShift()) break;
      ...
      if (this.shouldContextShift) continue;
  ...
  public async evaluateWithoutGeneratingNewTokens() {
    if (this.evaluationIterator != null) await this.evaluationIterator.return();

    await this.llamaChat.sequence.evaluateWithoutGeneratingNewTokens(
      this.tokens,
      removeNullFields({
  ...
  public async createNewEvaluationIterator() {
    ...
    this.evaluationIterator = this.llamaChat.sequence.evaluate(
      this.tokens,
      removeNullFields({
        ...
        grammarEvaluationState: () => {
          if (this.functionEvaluationMode !== false) return this.functionsEvaluationState;

          return this.grammarEvaluationState;
        },
        repeatPenalty: !this.repeatPenaltyEnabled
          ? undefined
          : {
  ...
  public async iterateEvaluation() {
    ...
    if (this.currentIteration != null && this.currentIteration?.done !== true) {
      ...
      else this.currentQueuedTokenRelease = undefined;
  ...
  public waitOnPartialCharactersOrWhiteSpaceTokens() {
    if (
      ...
      (this.currentText === '' &&
        this.locksToReleaseOnValidGeneration.length > 0 &&
      ...
      while (this.locksToReleaseOnValidGeneration.length > 0) this.locksToReleaseOnValidGeneration.shift()!.dispose();
    }
  }
  ...
  public detectAndHandleFunctionStartSyntax() {
    ...
    if (
      this.currentQueuedTokenRelease != null &&
      this.functionEvaluationMode === false &&
      this.functionsEnabled &&
  ...
  public recordStopGenerationEvaluation() {
    ...
    if (this.llamaChat.model.isEogToken(this.currentToken)) this.currentQueuedTokenRelease?.createTokenIndexLock(0);
  }
  ...
  public handleStopGenerationTrigger(lastHistoryItemType: 'user' | 'model') {
    if (
      this.stopGenerationDetector.hasTriggeredStops ||
      ...
      if (isEogToken || this.stopGenerationDetector.hasTriggeredStops) {
        return {
          ...
          metadata: {
            ...
            stopReason: isEogToken ? 'eogToken' : 'stopGenerationTrigger',
          },
  ...
  public moveFreePendingTokensToRes(removeFoundStartIgnoreTextsFromPendingTokens = true) {
    if (this.pendingTokens.length > 0 && (this.isMaxTokensTriggered() || !this.ignoreStartTextDetector.hasInProgressStops)) {
      if (removeFoundStartIgnoreTextsFromPendingTokens) this.removeFoundStartIgnoreTextsFromPendingTokens();
  ...
  public handleAbortTrigger(lastHistoryItemType: 'user' | 'model') {
    if (this.shouldAbort && this.signal?.aborted && this.stopOnAbortSignal) {
      if (this.res.length === 0) throw this.signal.reason;
