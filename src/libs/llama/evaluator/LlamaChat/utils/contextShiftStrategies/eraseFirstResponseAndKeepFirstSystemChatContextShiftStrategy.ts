...
import type { ChatHistoryItem, Tokenizer } from '../../../../types';
import { LlamaText } from '../../../../utils/LlamaText.js';
import { findCharacterRemovalCountToFitChatHistoryInContext } from '../../../../utils/findCharacterRemovalCountToFitChatHistoryInContext';
import { truncateLlamaTextAndRoundToWords, truncateTextAndRoundToWords } from '../../../../utils/truncateTextAndRoundToWords';
...
export async function eraseFirstResponseAndKeepFirstSystemChatContextShiftStrategy({
  ...
  if (isCalculationMetadata(lastShiftMetadata)) initialCharactersRemovalCount = lastShiftMetadata.removedCharactersNumber;
  ...
  const { removedCharactersCount, compressedChatHistory } = await findCharacterRemovalCountToFitChatHistoryInContext({
    ...
    failedCompressionErrorMessage:
      'Failed to compress chat history for context shift due to a too long prompt or system message that cannot be compressed without affecting the generation quality. ' +
    ...
    compressChatHistory({ chatHistory, charactersToRemove, estimatedCharactersPerToken }) {
      ...
      function removeHistoryThatLedToModelResponseAtIndex(index: number) {
        ...
        for (let i = index - 1; i >= 0; i--) {
          ...
          if (historyItem.type === 'model') break; // stop removing history items if we reach another model response

          if (i === 0 && historyItem.type === 'system') break; // keep the first system message
          ...
          if (historyItem.type === 'user' || historyItem.type === 'system') {
            ...
            if (newText.values.length === 0) {
              ...
              if (historyItem.type === 'user') historyItem.text = newText.toString();
              else historyItem.text = newText.toJSON();
            }
          } else {
      ...
      function compressHistoryThatLedToModelResponseAtIndex(index: number, keepTokensCount = 0) {
        let removedItems = 0;
        ...
        if (promptStartIndex == null || promptStartIndex >= index) return 0;

        for (let i = promptStartIndex; i < index && charactersLeftToRemove > 0; i++) {
          const historyItem = res[i];

          if (historyItem == null || historyItem.type !== 'user') continue;
          ...
          if (keepTokensCount > 0) {
            ...
            if (removeChars < 0) removeChars = 0;
      ...
      function compressFirstModelResponse() {
        for (let i = 0; i < res.length && charactersLeftToRemove > 0; i++) {
          ...
          for (let t = 0; t < historyItem.response.length && charactersLeftToRemove > 0; t++) {
            ...
            if (isLastHistoryItem && isLastText) continue;
            ...
            if (typeof item === 'string') {
              ...
              const functionCallAndResultTokenUsage = chatWrapper
                .generateFunctionCallsAndResults([item], true)
      ...
      if (charactersLeftToRemove <= 0) return res;
