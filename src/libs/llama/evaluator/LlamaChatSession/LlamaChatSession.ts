...
import type { ChatWrapper } from '../../ChatWrapper';
import type {
  ChatHistoryItem,
...
import type { LlamaText, LlamaTextJSON } from '../../utils/LlamaText';
import { appendUserMessageToChatHistory } from '../../utils/appendUserMessageToChatHistory';
import { safeEventCallback } from '../../utils/safeEventCallback';
import { wrapAbortSignal } from '../../utils/wrapAbortSignal';
import {
  type LLamaChatContextShiftOptions,
  LlamaChat,
  type LlamaChatResponse,
  type LlamaChatResponseFunctionCall,
} from '../LlamaChat/LlamaChat';
import type { LlamaContextSequence } from '../LlamaContext/LlamaContext';
import type { EvaluationPriority } from '../LlamaContext/types';
import type { LlamaGrammar } from '../LlamaGrammar.js';
import type { TokenBias } from '../TokenBias';
import {
  type LLamaChatPromptCompletionEngineOptions,
  LlamaChatSessionPromptCompletionEngine,
} from './utils/LlamaChatSessionPromptCompletionEngine';
...
export class LlamaChatSession {
  ...
  public get chatWrapper() {
    if (this._chat == null) throw new DisposedError();
   ...
   * @param [options]
   */
  public async promptWithMeta<const Functions extends ChatSessionModelFunctions | undefined = undefined>(
    prompt: string,
    {
      functions,
      documentFunctionParams,
    ...
    return await withLock(this._chatLock, 'evaluation', signal, async () => {
      ...
      let newContextWindowChatHistory =
        lastEvaluation?.contextWindow == null ? undefined : appendUserMessageToChatHistory(lastEvaluation?.contextWindow, prompt);

      const resolvedResponsePrefix = responsePrefix != null && responsePrefix !== '' ? responsePrefix : undefined;

      newChatHistory.push({
        type: 'model',
        response: resolvedResponsePrefix != null ? [resolvedResponsePrefix] : [],
      });
      ...
      try {
        while (true) {
          const functionCallsAndResults: Array<
            Promise<null | {
          ...
          const { lastEvaluation: currentLastEvaluation, metadata } = await this._chat.generateResponse<Functions>(newChatHistory, {
            functions,
            documentFunctionParams,
            maxParallelFunctionCalls,
              ...
              functionCallsAndResults.push(
                (async () => {
                  try {
                    ...
                    if (canThrowFunctionCallingErrors) throw err;
          ...
          this._ensureNotDisposed();
          if (abortController.signal.aborted && (abortedOnFunctionCallError || !stopOnAbortSignal)) throw abortController.signal.reason;
          ...
          if (functionCallsAndResults.length > 0) {
            ...
            await Promise.race([
              ...
              new Promise<void>((accept, reject) => {
                abortController.signal.addEventListener(
                  'abort',
                  () => {
                    if (abortedOnFunctionCallError || !stopOnAbortSignal) reject(abortController.signal.reason);
                    else accept();
                  },
            ...
            if (!abortController.signal.aborted) {
              const functionCallResults = (await functionCallResultsPromise).filter(
          ...
          const responseText = lastModelResponseItem.response.filter((item): item is string => typeof item === 'string').join('');
   ...
   * @param [options]
   */
  public async completePromptWithMeta(
    prompt: string,
    {
      maxTokens,
      stopOnAbortSignal = false,
    ...
    if (grammar != null) {
      if (grammar._llama == null) throw new Error('The grammar passed to this function is not a LlamaGrammar instance.');
      if (grammar._llama !== this.model._llama)
        throw new Error(
          "The LlamaGrammar used by passed to this function was created with a different Llama instance than the one used by this sequence's model. Make sure you use the same Llama instance for both the model and the grammar.",
        );
    }
    ...
    try {
      return await withLock(this._chatLock, 'evaluation', abortController.signal, async () => {
        ...
        if (!stopOnAbortSignal && metadata.stopReason === 'abort' && abortController.signal?.aborted) throw abortController.signal.reason;
  ...
  public getLastEvaluationContextWindow() {
    if (this._lastEvaluation == null) return null;
  ...
  public resetChatHistory() {
    if (this._chat == null || this.disposed) throw new DisposedError();

    const chatWrapperSupportsSystemMessages = this._chat.chatWrapper.settings.supportsSystemMessages;
    if (chatWrapperSupportsSystemMessages == null || chatWrapperSupportsSystemMessages || this._forceAddSystemPrompt)
      this.setChatHistory(this._chat.chatWrapper.generateInitialChatHistory({ systemPrompt: this._systemPrompt }));
    else this.setChatHistory([]);
  }

  /** @internal */
  private _stopAllPreloadAndPromptCompletions() {
    for (const abortController of this._preloadAndCompleteAbortControllers) abortController.abort();
...
function addFunctionCallToChatHistory({
  ...
  if (startsNewChunk) functionCall.startsNewChunk = true;
