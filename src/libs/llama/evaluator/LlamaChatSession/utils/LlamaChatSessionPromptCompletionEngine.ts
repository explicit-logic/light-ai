...
export class LlamaChatSessionPromptCompletionEngine {
  ...
  private _getCurrentCompletionCache() {
    ...
    if (completionCache != null) return completionCache;
  ...
  private _restartCompletion(completionCache: CompletionCache) {
    ...
    void this._chatSession
      .completePrompt(promptToComplete, {
        ...
        onTextChunk: (chunk) => {
          ...
          if (this._lastPrompt === prompt) this._onGeneration?.(prompt, completion);
        },
      })
      .then(() => {
      ...
      .catch((err) => {
        if (currentAbortSignal.aborted && err === currentAbortSignal.reason) return;
...
class CompletionCache {
  ...
  public getCompletion(input: string): string | null {
    ...
    for (let i = 0; i < input.length; i++) {
      ...
      if (!next.has(char)) {
        if (completion?.startsWith(input.slice(i))) {
  ...
  public putCompletion(input: string, completion: string): string {
    ...
    for (let i = 0; i < input.length; i++) {
      ...
      if (!next.has(char)) next.set(char, [new Map()]);

      node = next.get(char)!;
    }

    const currentCompletion = node[1];
    if (currentCompletion?.startsWith(completion)) return currentCompletion;
  ...
  private _deleteInput(input: string) {
    ...
    for (let i = 0; i < input.length; i++) {
      ...
      if (!next.has(char)) return;

      node = next.get(char)!;
    }

    if (lastNodeWithMultipleChildrenDeleteChar !== '') lastNodeWithMultipleChildren[0].delete(lastNodeWithMultipleChildrenDeleteChar);
  }
}

type InputNode = [next: Map<string, InputNode>, completion?: string];
