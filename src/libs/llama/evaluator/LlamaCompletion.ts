...
import type { LLamaContextualRepeatPenalty, Token } from '../types';
import type { LlamaText } from '../utils/LlamaText.js';
import { StopGenerationDetector } from '../utils/StopGenerationDetector.js';
import { type QueuedTokenReleaseLock, TokenStreamRegulator } from '../utils/TokenStreamRegulator.js';
...
import { tokenizeInput } from '../utils/tokenizeInput';
import { resolveBeginningTokenToPrepend } from '../utils/tokenizerUtils';
import type { LlamaContextSequence } from './LlamaContext/LlamaContext.js';
import type { EvaluationPriority } from './LlamaContext/types.js';
...
const defaultContextShiftSize = ((sequence) =>
  Math.max(1, Math.floor(sequence.context.contextSize / 10))) satisfies LlamaCompletionGenerationOptions['contextShiftSize'];
const defaultMinPrefixKeepTokens = ((sequence) =>
  Math.max(1, Math.floor(sequence.context.contextSize / 10))) satisfies LlamaInfillGenerationOptions['minPrefixKeepTokens'];
...
export class LlamaCompletion {
  ...
  public constructor({ contextSequence, autoDisposeSequence = false }: LlamaCompletionOptions) {
  ...
  public dispose({ disposeSequence = this._autoDisposeSequence }: { disposeSequence?: boolean } = {}) {
    if (this._sequence == null || this.disposed) return;
  ...
  public get infillSupported() {
    ...
    return this._sequence.model.tokens.infill.prefix != null && this._sequence.model.tokens.infill.suffix != null;
  }
  ...
  public async generateCompletionWithMeta(
    ...
    if (this._sequence == null || this.disposed) throw new DisposedError();
    ...
    async function fitInputIntoContext({
      ...
      if (beginningTokenToPrepend != null) res.push(beginningTokenToPrepend);
    ...
    return await withLock(this, 'generateCompletion', signal, async () => {
      ...
      const resolvedInput = tokenizeInput(
        ...
        beginningTokenToPrepend != null ? 'trimLeadingSpace' : undefined,
      );
  ...
  public async generateInfillCompletionWithMeta(
    ...
    if (prefixToken == null || suffixToken == null) throw new UnsupportedError('Infill completions are not supported by this model');
    ...
    async function fitInputIntoContext({
      ...
      const specialTokensInContext = 2 + (middleToken != null ? 1 : 0) + (beginningTokenToPrepend != null ? 1 : 0);
      ...
      const resolvedMinPrefixKeepTokens = Math.min(
        ...
        Math.max(1, Math.floor(minPrefixKeepTokens instanceof Function ? await minPrefixKeepTokens(sequence) : minPrefixKeepTokens)),
      );
      ...
      if (beginningTokenToPrepend != null) newContextState.push(beginningTokenToPrepend);
  ...
  private async _generateResponse(
    ...
    const grammarEvaluationState = grammar != null ? new LlamaGrammarEvaluationState({ model, grammar }) : undefined;
    const {
      ...
      frequencyPenalty,
      presencePenalty,
    }: LLamaContextualRepeatPenalty = repeatPenalty === false ? { lastTokens: 0 } : repeatPenalty;
    ...
    if (grammar != null)
      StopGenerationDetector.resolveStopTriggers(grammar.stopGenerationTriggers, model.tokenizer).map((stopTrigger) =>
        stopGenerationDetector.addStopTrigger(stopTrigger),
      );

    if (customStopTriggers != null)
      StopGenerationDetector.resolveStopTriggers(customStopTriggers, model.tokenizer).map((stopTrigger) =>
        customStopGenerationTriggersDetector.addStopTrigger(stopTrigger),
      );
    ...
    while (true) {
      ...
      const evaluationIterator = sequence.evaluate(
        inputTokens,
        removeNullFields({
          temperature,
          minP,
      ...
      for await (const token of evaluationIterator) {
        ...
        if (
          ...
          while (locksToReleaseOnValidGeneration.length > 0) locksToReleaseOnValidGeneration.shift()!.dispose();
        }
        ...
        if (model.isEogToken(token) || extraEosTokens.has(token)) queuedTokenRelease.createTokenIndexLock(0);

        pushAll(pendingTokens, streamRegulator.popFreeChunkTokens());

        if (
          stopGenerationDetector.hasTriggeredStops ||
          customStopGenerationTriggersDetector.hasTriggeredStops ||
          model.isEogToken(token) ||
          extraEosTokens.has(token)
        ) {
          ...
          if (grammar?.trimWhitespaceSuffix || trimWhitespaceSuffix) modelResponse = modelResponse.trimEnd();
...
async function resolveContextShiftSize(
  ...
  if (typeof contextShiftSize === 'number') return contextShiftSize;
  if (contextShiftSize instanceof Function)
    return Math.min(
      sequence.context.contextSize,
      Math.max(1, Math.floor(contextShiftSize instanceof Function ? await contextShiftSize(sequence) : contextShiftSize)),
    );
...
function getExtraCompletionEosTokens(model: LlamaModel) {
  ...
  if (
    model.fileInfo.metadata?.general?.architecture === GgufArchitectureType.gemma ||
    ...
    for (const token of model.iterateAllTokens()) {
      ...
      if (tokenText === '<|file_separator|>' || tokenText === '<|fim_prefix|>') {
        ...
        if (extraEosTokens.size === 2) break;
      }
    }
  }
