...
import type { AddonContext, AddonModelLora, BatchLogitIndex } from '../../bindings/AddonTypes.js';
import type { Llama } from '../../bindings/Llama.js';
import type { Token } from '../../types.js';
import { type DisposalPreventionHandle, DisposeGuard } from '../../utils/DisposeGuard.js';
import type { ThreadsSplitterConsumer } from '../../utils/ThreadsSplitter.js';
...
export class LlamaContext {
  ...
  private constructor(
    ...
    if (_model.disposed) throw new DisposedError();
    ...
    this._idealThreads =
      typeof threads === 'number'
        ...
        : this._llama._threadsSplitter.normalizeThreadsValue(
            threads?.ideal ?? (this._llama.maxThreads === 0 ? this._llama.cpuMathCores : this._llama.maxThreads),
          );
    this._minThreads = Math.max(1, typeof threads === 'number' ? 1 : this._llama._threadsSplitter.normalizeThreadsValue(threads?.min ?? 1));
    this._performanceTracking = !!performanceTracking;
    this._ctx = new this._llama._bindings.AddonContext(
      this._model._model,
      removeNullFields({
  ...
  public getAllocatedContextSize(): number {
    ...
    if (this._allocatedContextSize == null) this._allocatedContextSize = this._ctx.getContextSize();
  ...
  public getSequence(
    ...
    if (nextSequenceId == null) throw new Error('No sequences left');
  ...
  public dispatchPendingBatch() {
    ...
    if (this._batchDispatchPending) return;
    ...
    void withLock(this, 'context', async () => {
      ...
      const fitQueuedDecodesToABatch = (queuedDecodes: CurrentBatchItem[], batchSize: number) => {
        ...
        for (const { queuedDecode, processAmount } of queuedDecodes) {
          ...
          if (resolvedProcessAmount <= 0) {
            if (batchTokenSlotsLeft === 0) break;
      ...
      const decodeTokenBatchItems = async (batchItems: CurrentBatchItem[], currentBatchSize: number) => {
        ...
        if (currentBatchSize !== 0) this._ctx.initBatch(currentBatchSize);

        for (const { queuedDecode, processAmount } of batchItems) {
          let batchLogitIndex: ReturnType<typeof this._ctx.addToBatch>;
          try {
            const shouldGenerateLogitAtTheEnd = queuedDecode.generateLogitAtTheEnd && processAmount === queuedDecode.tokens.length;
        ...
        if (currentBatchSize !== 0) {
          ...
          const [threadsToUse, consumerHandle] =
            allocationResult instanceof Promise ? ((await allocationResult) ?? []) : (allocationResult ?? []);

          try {
            if (threadsToUse != null) this._ctx.setThreads(threadsToUse);
  ...
  public async printTimings() {
    ...
    if (!this._performanceTracking) throw new UnsupportedError('Performance tracking is not enabled');
  ...
  public async _decodeTokens<T>(
    ...
    onDone?: (batchLogitIndex: BatchLogitIndex) => T | Promise<T>,
  ): Promise<T> {
  ...
  private _popSequenceId(): number | null {
    if (this._unusedSequenceIds.length > 0) return this._unusedSequenceIds.shift()!;
  ...
  private _scheduleDecode() {
    if (this._dispatchDecodeScheduled || this._batchDispatchPending) return;

    this._dispatchDecodeScheduled = true;

    const currentPendingBatchHandle = this._currentDispatchBatchHandle;
    const dispatch = () => {
      if (this._currentDispatchBatchHandle !== currentPendingBatchHandle) return;
    ...
    if (this._queuedDecodeSequenceIds.size === this._totalSequences) dispatch();
    if (dispatchSchedule === 'nextTick') setTimeout(dispatch, 0);
    else dispatchSchedule(dispatch);
  }
  ...
  private _reserveThreads() {
    ...
    this._freeReservedThreadsTimeout = undefined;

    if (this._threadSplitterConsumer != null) return;
  ...
  private _freeReservedThreads() {
    ...
    this._threadSplitterConsumer = undefined;
  }
  ...
  public static async _create(
    ...
    const flashAttention = _model.flashAttentionSupported ? Boolean(options.flashAttention ?? _model.defaultContextFlashAttention) : false;
    const loraOptions =
      typeof options.lora === 'string'
        ? ({ adapters: [{ filePath: options.lora }] } satisfies LlamaContextOptions['lora'])
        : (options.lora satisfies LlamaContextOptions['lora']);
    let failedCreationRetries =
      options.failedCreationRemedy === false
        ? 0
        : Math.max(0, options.failedCreationRemedy?.retries ?? defaultFailedCreationRemedy.retries);
    const failedCreationAutoContextSizeShrink =
      options.failedCreationRemedy === false
        ? 0
    ...
    const minContextSize =
      options.contextSize === 'auto'
    ...
    async function createContext(contextSize: number) {
      ...
      try {
        if (createSignal?.aborted) throw createSignal.reason;

        const contextLoaded = await context._ctx.init();

        if (createSignal?.aborted) {
          if (contextLoaded) await context._ctx.dispose();

          throw createSignal.reason;
        }
        if (!contextLoaded) throw new Error('Failed to create context');
    ...
    while (failedCreationRetries >= 0) {
      try {
        ...
        if (failedCreationRetries === 0 || (createSignal?.aborted && err === createSignal.reason)) throw err;

        failedCreationRetries--;
        let newContextSize =
          typeof failedCreationAutoContextSizeShrink === 'number'
            ? Math.floor(contextSize * (1 - failedCreationAutoContextSizeShrink))
            : Math.floor(failedCreationAutoContextSizeShrink(contextSize));

        if (!Number.isFinite(newContextSize)) throw err;

        if (newContextSize < minContextSize) newContextSize = minContextSize;

        if (newContextSize >= contextSize) throw err;
...
export class LlamaContextSequence {
  ...
  /** @internal */ private _nextTokenIndex = 0;
  /** @internal */ private _disposed = false;
  ...
  public compareContextTokens(tokens: Token[]): {
    ...
    for (let i = 0; i < this._contextTokens.length; i++) {
      if (compareTokens(this._contextTokens[i], tokens[i])) continue;
   ...
   */
  public async adaptStateToTokens(tokens: Token[], allowShift = true) {
    ...
    for (let i = 0; i < this._contextTokens.length && tokensIndex < tokens.length; i++) {
      ...
      if (differentTokenIndex == null) differentTokenIndex = i;
    }
    ...
    if (eraseRanges.length > 0) await this.eraseContextTokenRanges(eraseRanges);
  }
    ...
    await withLock(this._context, 'context', async () => {
      ...
      const resolvedRanges = ranges
        .map(({ start, end }) => {
          ...
          if (end > this._nextTokenIndex) end = this._nextTokenIndex;

          if (start >= this._nextTokenIndex) return null;
  ...
  ): AsyncGenerator<Token, undefined | Token> {
    const {
      temperature = 0,
      minP = 0,
      topK = 40,
      ...
      tokenBias,
      evaluationPriority = 5,
      contextShift: { size: contextShiftSize = this._contextShift.size, strategy: contextShiftStrategy = this._contextShift.strategy } = {},
      yieldEogToken = false,
    ...
    try {
      while (true) {
        ...
        const nextToken: Token | null = await this._decodeTokens(
          ...
          contextShiftOptions,
          (batchLogitIndex) => {
            if (_noSampling) return null;

            const repeatPenaltyTokens =
              repeatPenalty?.punishTokens instanceof Function ? repeatPenalty.punishTokens() : repeatPenalty?.punishTokens;

            const maxPunishTokens = Math.max(repeatPenalty?.maxPunishTokens ?? defaultMaxPunishTokens, repeatPenaltyTokens?.length ?? 0);

            const resolvedGrammarEvaluationState =
              grammarEvaluationState instanceof Function ? grammarEvaluationState() : grammarEvaluationState;

            if (resolvedGrammarEvaluationState != null && resolvedGrammarEvaluationState._llama !== this.model._llama)
              throw new Error(
                "The LlamaGrammar used by passed to this function was created with a different Llama instance than the one used by this sequence's model. Make sure you use the same Llama instance for both the model and the grammar.",
              );
            ...
            sampler.applyConfig(
              removeNullFields({
                ...
                topK,
                topP,
                seed: Math.max(0, Number.isFinite(seed) ? Math.floor(seed ?? Date.now() / 1000) : Math.floor(Date.now() / 1000)),
                repeatPenalty: repeatPenalty?.penalty,
                repeatPenaltyMaxTokens: maxPunishTokens,
                repeatPenaltyTokens: repeatPenaltyTokens != null ? Uint32Array.from(repeatPenaltyTokens) : undefined,
            ...
            return withLock(sampler, 'sample', async () => {
              if (sampler.disposed) return null;
        ...
        if (nextToken === -1) throw new Error('Failed to sample next token');

        if (nextToken == null) return;

        // the model finished generating text
        if (!yieldEogToken && this._context.model.isEogToken(nextToken)) break;

        const replacementToken = (yield nextToken) as undefined | Token;

        // set the tokens for the next evaluation
        if (replacementToken != null) evalTokens = [replacementToken];
        else evalTokens = [nextToken];
      }
    } finally {
    ...
    return await withLock(this, 'evaluate', async (): Promise<T | null> => {
      while (tokensLeftToDecode.length > 0) {
        ...
        if (freeSpace <= 0) {
          ...
          if (freeSpace <= 0) throw new Error('Failed to free up space for new tokens');
        }
  ...
  private async _freeUpSpaceForTokens(contextShiftOptions: Required<ContextShiftOptions>) {
    ...
    const size = Math.min(
      ...
      Math.max(1, contextShiftOptions.size instanceof Function ? await contextShiftOptions.size(this) : contextShiftOptions.size),
    );
    ...
    if (contextShiftOptions.strategy === 'eraseBeginning') {
      ...
      if (this.model.tokens.bos != null && this._contextTokens[0] === this.model.tokens.bos) eraseStartIndex = 1;
      ...
      if (ranges == null) throw new Error('Invalid delete ranges');

      await this.eraseContextTokenRanges(ranges);

      if (this.nextTokenIndex >= this._context.contextSize - 1) await this.eraseContextTokenRanges([{ start: 0, end: size }]);
    }
  }
...
function getTokenBiasesForAddon(tokenBias: undefined | TokenBias | (() => TokenBias), currentModel: LlamaModel) {
  ...
  if (tokenBias instanceof Function) tokenBias = tokenBias();
...
function disposeContextIfReferenced(contextRef: WeakRef<LlamaContext>) {
  ...
  if (context != null) void context.dispose();
}
