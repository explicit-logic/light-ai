import type { BatchItem, PrioritizedBatchItem } from '../../types.js';

export function firstInFirstOutStrategy({ items, size }: { items: readonly BatchItem[]; size: number }) {
  const res: PrioritizedBatchItem[] = [];

  const sortedItems = items.slice().sort((a, b) => b.evaluationPriority - a.evaluationPriority);
