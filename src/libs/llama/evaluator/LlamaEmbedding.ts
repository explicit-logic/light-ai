...
export class LlamaEmbedding {
  ...
  public calculateCosineSimilarity(other: LlamaEmbedding | LlamaEmbeddingJSON | readonly number[]) {
    const otherVector = Array.isArray(other) ? other : other.vector;

    if (otherVector == null) throw new Error('Other vector is null');
    if (otherVector.length !== this.vector.length) {
      if (otherVector.length === 0 || this.vector.length === 0) return 0;
    ...
    for (let i = 0; i < this.vector.length; i++) {
      ...
      thisMagnitude += this.vector[i]! ** 2;
      otherMagnitude += otherVector[i]! ** 2;
    }

    if (thisMagnitude === 0 && otherMagnitude === 0) return 1;
    if (thisMagnitude === 0 || otherMagnitude === 0) return 0;
