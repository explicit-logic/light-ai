...
export class LlamaEmbeddingContext {
  ...
  public async getEmbeddingFor(input: Token[] | string | LlamaText) {
    ...
    if (beginningToken != null && resolvedInput[0] !== beginningToken) resolvedInput.unshift(beginningToken);

    const endToken = resolveEndTokenToAppend(this.model.vocabularyType, this.model.tokens);
    if (endToken != null && resolvedInput.at(-1) !== endToken) resolvedInput.push(endToken);
  ...
  public static async _create(
    ...
    { contextSize, batchSize, threads = 6, createSignal, ignoreMemorySafetyChecks }: LlamaEmbeddingContextOptions,
  ) {
