...
import { removeNullFields } from '@/libs/llama/utils/removeNullFields.js';
import { AsyncDisposeAggregator, DisposedError, EventRelay, withLock } from 'lifecycle-utils';
import type { AddonModel, AddonModelLora, ModelTypeDescription } from '../../bindings/AddonTypes.js';
import type { Llama } from '../../bindings/Llama.js';
import { LlamaLocks, LlamaLogLevel, LlamaVocabularyType, LlamaVocabularyTypeValues } from '../../bindings/types';
import { maxRecentDetokenizerTokens } from '../../consts';
import { GgufInsights } from '../../gguf/insights/GgufInsights.js';
import { readGgufFileInfo } from '../../gguf/readGgufFileInfo.js';
import type { GgufFileInfo } from '../../gguf/types/GgufFileInfoTypes';
import { GgufArchitectureType, type GgufMetadata } from '../../gguf/types/GgufMetadataTypes';
import type { Token, Tokenizer } from '../../types';
import { type DisposalPreventionHandle, DisposeGuard } from '../../utils/DisposeGuard.js';
import type { BuiltinSpecialTokenValue } from '../../utils/LlamaText';
import type { OverridesObject } from '../../utils/OverridesObject';
import { getConsoleLogPrefix } from '../../utils/getConsoleLogPrefix';
import { getReadablePath } from '../../utils/getReadablePath';
import type { Writable } from '../../utils/utilTypes.js';
import { LlamaContext } from '../LlamaContext/LlamaContext.js';
import type { LlamaContextOptions } from '../LlamaContext/types.js';
import { LlamaEmbeddingContext, type LlamaEmbeddingContextOptions } from '../LlamaEmbeddingContext.js';
...
export class LlamaModel {
  ...
  private constructor(
    ...
    this._model = new this._llama._bindings.AddonModel(
      this._modelPath,
      removeNullFields({
        ...
        useMmap,
        useMlock: _llama.supportsMlock ? useMlock : undefined,
        ...
        overridesList: overridesList.length > 0 ? overridesList : undefined,
      }),
    );
  ...
  public async dispose() {
    if (this._disposedState.disposed) return;
  ...
  public tokenize(text: string, specialTokens: boolean | 'builtin' = false, options?: 'trimLeadingSpace'): Token[] {
    ...
    if (specialTokens === 'builtin') {
      ...
      switch (builtinToken) {
        case 'BOS':
          return this.tokens.bos == null ? [] : [this.tokens.bos];
        case 'EOS':
          return this.tokens.eos == null ? [] : [this.tokens.eos];
        case 'NL':
          return this.tokens.nl == null ? [] : [this.tokens.nl];
        case 'EOT':
          return this.tokens.eot == null ? [] : [this.tokens.eot];
      }
    ...
    if (options === 'trimLeadingSpace') {
      if (specialTokens) {
        ...
        const [workaroundToken, workaroundTokenString] =
          this.tokens.bos != null && this.tokens.bosString != null
        ...
        if (workaroundToken != null && workaroundTokenString != null) {
          ...
          if (workaroundTokenIndex >= 0 && workaroundTokenIndex <= 1) {
            ...
            if (countLeadingSpaces(this.detokenize(tokens, true)) === textLeadingSpaces) return tokens;
          }
        }
        ...
        if (text.startsWith(workaroundTokensString)) {
          ...
          if (this.detokenize(tokens, true).startsWith(workaroundTokensString)) return tokens;
        }
        ...
        if (text.startsWith(workaroundTokensString)) {
          ...
          if (this.detokenize(tokens, false).startsWith(workaroundTokensString)) return tokens;
        }
   ...
   */
  public detokenize(tokens: readonly Token[], specialTokens = false, lastTokens?: readonly Token[]): string {
    this._ensureNotDisposed();

    if (tokens.length === 0) return '';

    if (lastTokens == null || lastTokens.length === 0) return this._model.detokenize(Uint32Array.from(tokens), Boolean(specialTokens));
    ...
    if (addedTokensText === '') return this._model.detokenize(Uint32Array.from(tokens), Boolean(specialTokens));

    const text = this._model.detokenize(Uint32Array.from([...addedTokens, ...tokens]), Boolean(specialTokens));
    if (text.startsWith(addedTokensText)) return text.slice(addedTokensText.length);
  ...
  public getTokenAttributes(token: Token): TokenAttributes {
    ...
    if (this.vocabularyType === LlamaVocabularyType.none) return TokenAttributes._create(token, TokenAttribute.undefined);
  ...
  public isSpecialToken(token: Token | undefined): boolean {
    ...
    if (this.getTokenAttributes(token).control) return true;

    const normalText = this.detokenize([token], false);

    if (normalText === '') return this.detokenize([token], true) !== '';

    return false;
  }

  public *iterateAllTokens() {
    if (this.vocabularyType === LlamaVocabularyType.none) return;

    const totalTokens = this.fileInfo.metadata?.tokenizer?.ggml?.tokens?.length;
    if (typeof totalTokens !== 'number') return;

    for (let i = 0; i < totalTokens; i++) yield i as Token;
  }
  ...
  public async createContext(options: LlamaContextOptions = {}) {
    if (this._vocabOnly) throw new Error('Model is loaded in vocabOnly mode, so no context can be created');
  ...
  public getWarnings() {
    ...
    try {
      ...
      if (beforeTextNoSpecialTokens !== afterTextNoSpecialTokens)
        warnings.push(
          `Using this model ${modelFilePathText} to tokenize text and then detokenize it resulted in a different text. There might be an issue with the model or the tokenizer implementation. Using this model may not work as intended`,
        );
    } catch (err) {
      // do nothing
    }
    ...
    try {
      if (this._defaultContextFlashAttentionOptionEnabled && !this._flashAttentionSupported) {
        if (this.fileInfo.metadata?.general?.architecture === GgufArchitectureType.grok)
          ...
          if (nEmbdHeadK !== nEmbdHeadV) warnings.push('Flash attention is incompatible with this model and thus was turned off');
        }
      }
    } catch (err) {
      // do nothing
  ...
  public get typeDescription(): ModelTypeDescription {
    ...
    if (this._typeDescription == null) this._typeDescription = this._model.getModelDescription();
  ...
  public get trainContextSize(): number {
    ...
    if (this._trainContextSize == null) this._trainContextSize = this._model.getTrainContextSize();
  ...
  public get embeddingVectorSize(): number {
    ...
    if (this._embeddingVectorSize == null) this._embeddingVectorSize = this._model.getEmbeddingVectorSize();
  ...
  private _ensureNotDisposed() {
    if (this._disposedState.disposed) throw new DisposedError();
  }

  /** @internal */
  public async _getOrLoadLora(filePath: string) {
    const resolvedPath = path.resolve(process.cwd(), filePath);
    if (this._loraAdapters.has(resolvedPath)) return this._loraAdapters.get(resolvedPath)!;
  ...
  public static async _create(
    ...
    const model = new LlamaModel(
      { ...modelOptions, gpuLayers, useMmap },
      {
        _fileInfo: fileInfo,
        _fileInsights: ggufInsights,
        _llama,
    ...
    try {
      ...
      if (loadSignal?.aborted) {
        if (modelLoaded) await model._model.dispose();
...
export class LlamaModelTokens {
  ...
  public get infill() {
    ...
    if (this._infillTokens == null) this._infillTokens = LlamaModelInfillTokens._create(this._model, this._disposedState);
  ...
  public get bos(): Token | null {
    ...
    if (this._bosToken == null) this._bosToken = this._model.tokenBos();

    if (this._bosToken === -1) return null;
  ...
  public get eos(): Token | null {
    ...
    if (this._eosToken == null) this._eosToken = this._model.tokenEos();

    if (this._eosToken === -1) return null;
  ...
  public get eot(): Token | null {
    ...
    if (this._eotToken == null) this._eotToken = this._model.eotToken();

    if (this._eotToken === -1) return null;
  ...
  public get cls(): Token | null {
    ...
    if (this._clsToken == null) this._clsToken = this._model.clsToken();

    if (this._clsToken === -1) return null;
  ...
  public get sep(): Token | null {
    ...
    if (this._sepToken == null) this._sepToken = this._model.sepToken();

    if (this._sepToken === -1) return null;
  ...
  public get nl(): Token | null {
    ...
    if (this._nlToken == null) this._nlToken = this._model.tokenNl();

    if (this._nlToken === -1) return null;
  ...
  public get bosString(): string | null {
    ...
    if (this._bosString == null) this._bosString = this._model.getTokenString(bosToken);
  ...
  public get eosString(): string | null {
    ...
    if (eosToken == null) return null;

    if (this._eosString == null) this._eosString = this._model.getTokenString(eosToken);
  ...
  public get eotString(): string | null {
    ...
    if (eotToken == null) return null;

    if (this._eotString == null) this._eotString = this._model.getTokenString(eotToken);
  ...
  public get clsString(): string | null {
    ...
    if (clsToken == null) return null;

    if (this._clsString == null) this._clsString = this._model.getTokenString(clsToken);
  ...
  public get sepString(): string | null {
    ...
    if (sepToken == null) return null;

    if (this._sepString == null) this._sepString = this._model.getTokenString(sepToken);
  ...
  public get nlString(): string | null {
    ...
    if (nlToken == null) return null;

    if (this._nlString == null) this._nlString = this._model.getTokenString(nlToken);
  ...
  public get shouldPrependBosToken(): boolean {
    ...
    if (this._shouldPrependBosToken == null) this._shouldPrependBosToken = this.bos != null && this._model.shouldPrependBosToken();
  ...
  public get shouldAppendEosToken(): boolean {
    ...
    if (this._shouldAppendEosToken == null) this._shouldAppendEosToken = this.bos != null && this._model.shouldAppendEosToken();
...
export class LlamaModelInfillTokens {
  ...
  public get prefix(): Token | null {
    ...
    if (this._prefixToken == null) this._prefixToken = this._resolveSpecialToken(this._model.prefixToken(), ['<fim_prefix>']);

    if (this._prefixToken === -1) return null;
  ...
  public get middle(): Token | null {
    ...
    if (this._middleToken == null) this._middleToken = this._resolveSpecialToken(this._model.middleToken(), ['<fim_middle>']);

    if (this._middleToken === -1) return null;
  ...
  public get suffix(): Token | null {
    ...
    if (this._suffixToken == null) this._suffixToken = this._resolveSpecialToken(this._model.suffixToken(), ['<fim_suffix>']);

    if (this._suffixToken === -1) return null;
  ...
  public get prefixString(): string | null {
    ...
    if (prefixToken == null) return null;

    if (this._prefixString == null) this._prefixString = this._model.getTokenString(prefixToken);
  ...
  public get middleString(): string | null {
    ...
    if (middleToken == null) return null;

    if (this._middleString == null) this._middleString = this._model.getTokenString(middleToken);
  ...
  public get suffixString(): string | null {
    ...
    if (suffixToken == null) return null;

    if (this._suffixString == null) this._suffixString = this._model.getTokenString(suffixToken);
  ...
  private _resolveSpecialToken(token: Token, fallbackTexts: string[]): Token {
    if (token != null && token !== -1) return token;
...
function applyGgufMetadataOverrides(
  ...
  function applyOverride(object: object, override?: object) {
    if (override == null || object == null) return;

    if (Array.isArray(object) || typeof object !== 'object' || typeof override !== 'object') return;

    for (const [key, value] of Object.entries(override)) {
      if (Array.isArray(value) || typeof value !== 'object' || (typeof value === 'object' && typeof (object as any)[key] !== 'object'))
        (object as any)[key] = value;
      else applyOverride((object as any)[key], value);
    }
  }
...
function ggufMetadataOverridesToList(overrides?: OverridesObject<GgufMetadata, number | bigint | boolean | string>) {
  ...
  const res: Array<[key: string, value: number | bigint | boolean | string, type: 0 | 1 | undefined]> = [];

  function addItem(object: number | bigint | boolean | string | object, path: string[]) {
    if (object == null || Array.isArray(object)) return;

    if (typeof object !== 'object') {
      if (typeof object === 'string' && object.length > maxStringLength)
        throw new Error(
          `Metadata key "${path.join('.')}" override string value (${JSON.stringify(object)}) is longer than ${maxStringLength} characters`,
        );

      const key = path.join('.');
      if (key.length > maxKeyLength) throw new Error(`Metadata key "${key}" override path is longer than ${maxKeyLength} characters`);

      let type: 0 | 1 | undefined = undefined;
      if (typeof object === 'number') {
        if (typeof object === 'bigint' || Number.isInteger(object)) type = 0;
        else type = 1;
      }
    ...
    for (const [key, value] of Object.entries(object)) addItem(value, [...path, key]);
  }
