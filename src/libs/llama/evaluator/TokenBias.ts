...
export class TokenBias {
  ...
  public set(input: Token | Token[] | string | LlamaText, bias: 'never' | number | { logit: number }) {
    const resolvedLogit = bias === 'never' ? Number.NEGATIVE_INFINITY : typeof bias === 'number' ? probabilityToLogit(bias) : bias.logit;

    for (const token of tokenizeInput(input, this._tokenizer)) {
      if (this._tokenizer.isEogToken(token)) continue;
  ...
  public static for(modelOrTokenizer: LlamaModel | Tokenizer) {
    if ((modelOrTokenizer as LlamaModel).tokenizer != null) return new TokenBias((modelOrTokenizer as LlamaModel).tokenizer);
...
function probabilityToLogit(probability: number) {
  if (probability <= -1) return Number.NEGATIVE_INFINITY;
