...
export class TokenMeter {
  ...
  public useTokens(tokens: number, type: 'input' | 'output') {
    if (tokens < 0) throw new RangeError('Tokens cannot be negative');
    if (tokens === 0) return;

    if (type === 'input') this._inputTokens += tokens;
    else if (type === 'output') this._outputTokens += tokens;
    else {
  ...
  public static useTokens(
    ...
    if (meters instanceof TokenMeter) meters.useTokens(tokens, type);
    else {
      for (const meter of meters) meter.useTokens(tokens, type);
    }
  }
   ...
   */
  public static diff(meter1: TokenMeter | TokenMeterState, meter2: TokenMeter | TokenMeterState) {
    return {
