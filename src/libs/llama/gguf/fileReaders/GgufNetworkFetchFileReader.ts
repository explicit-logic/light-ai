...
import { type ModelFileAccessTokens, resolveModelFileAccessTokensTryHeaders } from '../../utils/modelFileAccesTokens.js';
...
export class GgufNetworkFetchFileReader extends GgufFileReader {
  ...
  public readByteRange(offset: number | GgufReadOffset, length: number) {
    ...
    if (endOffset >= this._buffer.length)
      return this._fetchToExpandBufferUpToOffset(endOffset).then(() => {
  ...
  protected ensureHasByteRange(offset: number | GgufReadOffset, length: number) {
    ...
    if (endOffset >= this._buffer.length)
      return this._fetchToExpandBufferUpToOffset(endOffset).then(() => {
        if (endOffset >= this._buffer.length) throw new Error('Expected buffer to be long enough for the requested byte range');
      });
  ...
  private async _fetchToExpandBufferUpToOffset(endOffset: number, extraAllocationSize: number = defaultExtraAllocationSize) {
    await withLock(this, 'modifyBuffer', this._signal, async () => {
      if (endOffset < this._buffer.length) return;
      ...
      if (this._signal?.aborted) throw this._signal.reason;
  ...
  private async _fetchByteRange(start: number, length: number): Promise<Buffer> {
    if (this._tryHeaders == null) this._tryHeaders = await resolveModelFileAccessTokensTryHeaders(this.url, this.tokens, this.headers);
    ...
    while (headersToTry.length > 0) {
      ...
      if ((response.status >= 500 || response.status === 429) && headersToTry.length > 0) continue;

      if (!response.ok) throw new Error(`Failed to fetch byte range: ${response.status} ${response.statusText}`);
