import { getLlamaWithoutBackend } from '@/utils/llama/getLlamaWithoutBackend';
...
import type { GgufFileInfo } from '../types/GgufFileInfoTypes';
import { GgufArchitectureType } from '../types/GgufMetadataTypes';
import type { GgufTensorInfo } from '../types/GgufTensorInfoTypes';
...
export class GgufInsights {
  ...
  public getWarnings(modelFilePath?: string) {
    ...
    const modelFilePathText = modelFilePath != null && modelFilePath !== '' ? ` ("${getReadablePath(modelFilePath)}")` : '';

    if (this._ggufFileInfo?.metadata?.tokenizer?.ggml?.model === 'gpt2' && this._ggufFileInfo?.metadata?.tokenizer?.ggml?.model == null) {
      // equivalent to the warning in `llama.cpp` under `llm_load_vocab`: "missing pre-tokenizer type, using: 'default'"
      warnings.push(
        `This model file${modelFilePathText} is missing a pre-tokenizer configuration. This may cause incorrect tokenization and thus degrade the generation quality. Consider using a newer model or regenerating this GGUF model file`,
      );
    }
  ...
  public get totalLayers() {
    if (this._totalLayers != null) return this._totalLayers;
  ...
  public get flashAttentionSupported() {
    ...
    if (this._ggufFileInfo.metadata?.general?.architecture === GgufArchitectureType.grok) return false;
    if (this._ggufFileInfo.metadata?.general?.architecture === GgufArchitectureType.gemma2) return false;
    ...
    if (nEmbdHeadK !== nEmbdHeadV) return false;
  ...
  public estimateContextResourceRequirements({
    ...
    const embedSize = isEmbeddingContext ? (llmData.embedding_length ?? 0) * batchSize : 0;
    ...
    const kvSelfLength =
      this._ggufFileInfo.metadata.general?.architecture === GgufArchitectureType.mamba ? Math.max(1, sequences) : actualContextSize;
    ...
    const estimateGraphOverheadMemory = () => {
      ...
      if (this._ggufFileInfo.metadata.general?.architecture === GgufArchitectureType.qwen2) {
        if (modelGpuLayers === this.totalLayers) {
          defaultCalculationAdjustment -= s1MB * 340 * (this.trainContextSize == null ? 1 : actualContextSize / this.trainContextSize);
        } else {
          defaultCalculationAdjustment -=
            s1MB * 250 + s1MB * 50 * (this.trainContextSize == null ? 1 : actualContextSize / this.trainContextSize);
        }
        ...
        if (modelGpuLayers === this.totalLayers) {
          defaultCalculationAdjustment +=
            s1MB * 40 - s1MB * 270 * (this.trainContextSize == null ? 1 : actualContextSize / this.trainContextSize);
        } else {
          defaultCalculationAdjustment +=
            -(s1MB * 550) + s1MB * 150 * (this.trainContextSize == null ? 1 : Math.max(0, 1 - actualContextSize / this.trainContextSize));
        }
      ...
      const totalElements =
        tensorInfo.length === 0
          ? this.totalLayers * (((llmData.embedding_length ?? 0) + (llmData.feed_forward_length ?? 0)) / 2)
    ...
    const graphOverheadMemory = flashAttention || !includeGraphOverhead ? 0 : estimateGraphOverheadMemory();

    const usingGpu = finalGpuLayers !== 0;

    const cpuRam =
      (!usingGpu ? overheadMemory + graphOverheadMemory : 0) + this._estimateKvMemorySizeInBytes(actualContextSize, finalCpuLayers);
    const gpuVram = usingGpu
      ? overheadMemory +
        graphOverheadMemory +
        this._estimateKvMemorySizeInBytes(actualContextSize, finalGpuLayers < totalLayers ? finalGpuLayers + 1 : finalGpuLayers)
      : 0;
    ...
    for (const singleTensorInfo of tensorInfo) {
      ...
      if (gpuLayers !== this.totalLayers) {
        ...
        if (architecture === GgufArchitectureType.qwen2 || architecture === GgufArchitectureType.gemma) {
          if (layerNumber != null && layerNumber >= startGpuLayer) gpuTensors.push(singleTensorInfo);
          else cpuTensors.push(singleTensorInfo);
  ...
  public _determineNumberOfLayersFromTensorInfo(): number {
    ...
    for (const singleTensorInfo of this._ggufFileInfo.fullTensorInfo ?? []) {
      ...
      if (layerNumber != null) layerNumbers.add(layerNumber);
    }
  ...
  public _estimateKvMemorySizeInBytes(contextSize: number, layers: number) {
    ...
    const keyTypeSize =
      this._ggufFileInfo.metadata.general?.architecture === GgufArchitectureType.mamba
  ...
  public static async from(ggufFileInfo: GgufFileInfo, llama?: Llama) {
    ...
    if (resolvedLlama == null) resolvedLlama = await getLlamaWithoutBackend();
...
function parseTensorName(tensorName?: string): {
  ...
  if (tensorName == null) return { layerNumber: undefined };

  const layerTensorPrefix = 'blk.';
  if (!tensorName.startsWith(layerTensorPrefix)) return { layerNumber: undefined };

  const dotIndex = tensorName.indexOf('.', layerTensorPrefix.length);
  const layerNumberString = tensorName.slice(layerTensorPrefix.length, dotIndex < 0 ? tensorName.length : dotIndex);

  const layerNumber = Number.parseInt(layerNumberString);
  if (Number.isFinite(layerNumber)) return { layerNumber };
...
function calculateTensorsSize(tensorsInfo: GgufTensorInfo[], llama: Llama) {
  let size = 0;
  for (const tensorInfo of tensorsInfo) size += calculateTensorSize(tensorInfo, llama);
...
function calculateTensorSize(tensor: GgufTensorInfo, llama: Llama) {
  ...
  if (typeSize == null || blockSize == null) throw new Error('Invalid type or block size');
...
function getTensorNeAndNb(
  ...
  const ne = [...tensor.dimensions, ...Array(Math.max(0, ggmlMaxDims - tensor.dimensions.length)).fill(1)].slice(0, ggmlMaxDims);

  // number of bytes
  // source: `ggml_new_tensor_impl` in `ggml.c`
  const nb = [typeSize, Math.floor(typeSize * (ne[0] / blockSize)), ...Array(ggmlMaxDims - 2).fill(0)];
