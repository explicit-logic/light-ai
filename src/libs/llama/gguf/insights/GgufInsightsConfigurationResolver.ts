...
import { InsufficientMemoryError } from '../../utils/InsufficientMemoryError';
...
export class GgufInsightsConfigurationResolver {
  ...
  public async scoreModelConfigurationCompatibility(
    ...
    let resolvedGpuLayers = forceGpuLayers == null || forceGpuLayers === 'max' ? this.ggufInsights.totalLayers : forceGpuLayers;
          ...
          : embeddingContext
            ? {
                fitContext: {
                  ...
                  contextSize: forceStrictContextSize ? contextSize : undefined,
                },
              }
    ...
    const contextSizePoints = contextFitsMemory ? rankPoints.contextSize * Math.min(1, resolvedContextSize / contextSize) : 0;
    const ramUsageFitsInRamPoints =
      rankPoints.ramUsageFitsInRam *
    ...
    if (
      ...
      bonusScore =
        (1 -
          totalVramRequirement / (vramState.total * maximumUnfitConfigurationResourceMultiplier) +
  ...
  public async resolveModelGpuLayers(
   ...
   */
  public async resolveContextContextSize(
