...
export async function resolveContextContextSizeOption({
  ...
  if (contextSize == null) contextSize = 'auto';

  if (typeof contextSize === 'number') {
    const resolvedContextSize = Math.max(1, Math.floor(contextSize));

    if (ignoreMemorySafetyChecks) return resolvedContextSize;

    const [vramState, ramState, swapState] = await Promise.all([getVramState(), getRamState(), getSwapState()]);
    ...
    if (contextResourceRequirements.gpuVram > vramState.free)
      throw new InsufficientMemoryError(
        `A context size of ${resolvedContextSize}${sequences > 1 ? ` with ${sequences} sequences` : ''} is too large for the available VRAM`,
      );
    if (
      ...
      throw new InsufficientMemoryError(
        `A context size of ${resolvedContextSize}${sequences > 1 ? ` with ${sequences} sequences` : ''} is too large for the available RAM${swapState.total > 0 ? ' (including swap)' : ''}`,
      );

    return resolvedContextSize;
  }
  if (contextSize === 'auto' || typeof contextSize === 'object') {
    const [vramState, ramState, swapState] = await Promise.all([getVramState(), getRamState(), getSwapState()]);
    ...
    for (let testContextSize = maxContextSize; testContextSize >= minContextSize && testContextSize <= maxContextSize; ) {
      ...
      if (
        contextResourceRequirements.gpuVram <= vramState.free &&
        contextResourceRequirements.cpuRam <=
          ramState.free -
            getRamUsageFromUnifiedVram(contextResourceRequirements.gpuVram, vramState) +
            (testContextSize <= maxContextSizeSwapUse ? swapState.free : 0)
      ) {
        if (highestCompatibleContextSize == null || testContextSize >= highestCompatibleContextSize) {
          ...
          if (step === -1) break;
          if (step < 0) step = Math.max(1, Math.floor(-step / 2));
        }
      } else if (step > 0) step = -Math.max(1, Math.floor(step / 2));

      if (testContextSize === minContextSize && step === -1) break;
    ...
    if (highestCompatibleContextSize != null) return highestCompatibleContextSize;

    if (ignoreMemorySafetyChecks) return minContextSize;
    ...
    if (
      minContextSizeResourceRequirements.gpuVram > vramState.free &&
      minContextSizeResourceRequirements.cpuRam > ramState.free + swapState.free - unifiedRamUsage
    )
      throw new InsufficientMemoryError(
        `A context size of ${minContextSize}${sequences > 1 ? ` with ${sequences} sequences` : ''} is too large for the available VRAM and RAM${swapState.total > 0 ? ' (including swap)' : ''}`,
      );
    if (minContextSizeResourceRequirements.gpuVram > vramState.free)
      throw new InsufficientMemoryError(
        `A context size of ${minContextSize}${sequences > 1 ? ` with ${sequences} sequences` : ''} is too large for the available VRAM`,
      );
    if (minContextSizeResourceRequirements.cpuRam > ramState.free + swapState.free - unifiedRamUsage)
      throw new InsufficientMemoryError(
        `A context size of ${minContextSize}${sequences > 1 ? ` with ${sequences} sequences` : ''} is too large for the available RAM${swapState.total > 0 ? ' (including swap)' : ''}`,
      );
    if (minContextSizeResourceRequirements.cpuRam > ramState.free - unifiedRamUsage)
      throw new InsufficientMemoryError(
        `A context size of ${minContextSize}${sequences > 1 ? ` with ${sequences} sequences` : ''} is too large for the available RAM`,
      );

    throw new InsufficientMemoryError(
      `A context size of ${minContextSize}${sequences > 1 ? ` with ${sequences} sequences` : ''} is too large for the available resources`,
    );
  }
