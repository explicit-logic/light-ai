import type { BuildGpu } from '../../../bindings/types';
import { minAllowedContextSizeInCalculations } from '../../../config';
import { getDefaultContextBatchSize, getDefaultModelContextSize } from '../../../evaluator/LlamaContext/LlamaContext.js';
import type { LlamaModelOptions } from '../../../evaluator/LlamaModel/LlamaModel.js';
import { InsufficientMemoryError } from '../../../utils/InsufficientMemoryError.js';
import { findBestOption } from '../../../utils/findBestOption';
...
export async function resolveModelGpuLayersOption(
  gpuLayers: LlamaModelOptions['gpuLayers'],
  {
    ggufInsights,
  ...
  if (gpuLayers == null) gpuLayers = 'auto';

  if (!llamaSupportsGpuOffloading) return 0;

  if (gpuLayers === 'max' || typeof gpuLayers === 'number') {
    const resolvedGpuLayers =
      typeof gpuLayers === 'number' ? Math.max(0, Math.min(ggufInsights.totalLayers, gpuLayers)) : ggufInsights.totalLayers;

    if (ignoreMemorySafetyChecks) return resolvedGpuLayers;
    ...
    if (maxLayersRequirements == null) throw new InsufficientMemoryError('Not enough VRAM to fit the model with the specified settings');

    return resolvedGpuLayers;
  }
  if (gpuLayers === 'auto' || typeof gpuLayers === 'object') {
    if (llamaGpu === false) return 0;
    ...
    if (typeof gpuLayers === 'object' && gpuLayers.fitContext?.contextSize != null) {
      ...
      if (freeVram < 0) freeVram = 0;
    }

    const bestGpuLayersOption = getBestGpuLayersForFreeVram({
      ggufInsights,
      freeVram,
      fitContext: typeof gpuLayers === 'object' ? gpuLayers.fitContext : undefined,
      minGpuLayers: typeof gpuLayers === 'object' ? gpuLayers.min : undefined,
      maxGpuLayers: typeof gpuLayers === 'object' ? gpuLayers.max : undefined,
      defaultContextFlashAttention,
    });

    const hasGpuLayersRequirements =
      typeof gpuLayers === 'object' && (gpuLayers.min != null || gpuLayers.max != null || gpuLayers.fitContext?.contextSize != null);
    ...
    findBestOption({
      *generator() {
      ...
      score(option) {
        ...
        if (layersRequirements == null) return null;

        return scoreGpuLayersAndContextCombination(
          { gpuLayers: option.gpuLayers, contextSize: layersRequirements.contextSize },
          {
...
function scoreGpuLayersAndContextCombination(
  { gpuLayers, contextSize }: { gpuLayers: number; contextSize: number },
  {
    totalGpuLayers,
    trainContextSize,
  }: {
...
function getVramRequiredForGpuLayers({
  ...
  if (modelVram > currentVram) return null;
  ...
  if (fitContext != null && fitContext.contextSize != null) {
    ...
    if (totalVram > currentVram) return null;
  ...
  if (maxContext == null || modelVram + maxContext.vram > currentVram) return null;
