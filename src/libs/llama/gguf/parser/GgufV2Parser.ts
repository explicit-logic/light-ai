import { type Promisable, promisableLoop, transformPromisable, transformPromisables } from '../../utils/transformPromisable.js';
...
import {
  GgufValueType,
  type GgufVersionParserOptions,
  type GgufVersionParserResult,
  type MetadataKeyValueRecord,
  type MetadataValue,
} from '../types/GgufFileInfoTypes.js';
import type { GgufMetadata } from '../types/GgufMetadataTypes.js';
import type { GgmlType, GgufTensorInfo } from '../types/GgufTensorInfoTypes.js';
...
export class GgufV2Parser {
  ...
  public async parse(): Promise<GgufVersionParserResult> {
    ...
    const headerReadResult = headerReadResultPromisable instanceof Promise ? await headerReadResultPromisable : headerReadResultPromisable;
    const tensorReadResultPromisable = this._shouldReadTensorInfo
      ...
      : null;
    const tensorReadResult = tensorReadResultPromisable instanceof Promise ? await tensorReadResultPromisable : tensorReadResultPromisable;
  ...
  protected _readGgufValue(type: GgufValueType, offset: number | GgufReadOffset): Promisable<MetadataValue> {
    ...
    switch (type) {
      ...
      case GgufValueType.String:
        return this._readStringValue(readOffset);
