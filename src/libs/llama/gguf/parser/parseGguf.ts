...
import type { GgufFileInfo, GgufVersionParserOptions, GgufVersionParserResult } from '../types/GgufFileInfoTypes.js';
...
async function parseMagicAndVersion(fileReader: GgufFileReader, readOffset: GgufReadOffset) {
  ...
  if (fileMagicText !== ggufMagic) throw new InvalidGgufMagicError(ggufMagic, fileMagicText);
  ...
  switch (specificVersionParserOptions.version) {
    ...
    default:
      if (specificVersionParserOptions.logWarnings)
        console.warn(
          `${getConsoleLogPrefix()}Unsupported GGUF version "${specificVersionParserOptions.version}". Reading the file as GGUF version 3`,
        );
