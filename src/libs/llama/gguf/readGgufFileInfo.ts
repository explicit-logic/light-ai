import type retry from '../../../@types/async-retry/index.js';
import { isUrl } from '../utils/isUrl';
import type { ModelFileAccessTokens } from '../utils/modelFileAccesTokens';
...
import type { GgufFileInfo } from './types/GgufFileInfoTypes.js';
 ...
 * @param options
 */
export async function readGgufFileInfo(
  pathOrUri: string,
  {
    readTensorInfo = true,
    sourceType,
  ...
  function createFileReader(pathOrUri: string) {
    ...
    if (sourceType === 'filesystem' || sourceType == null) {
  ...
  if (!spliceSplitFiles) return await readSingleFile(pathOrUri);

  const allSplitPartPaths = resolveSplitGgufParts(pathOrUri);

  if (allSplitPartPaths.length === 1) return await readSingleFile(allSplitPartPaths[0]!);

  const [first, ...rest] = await Promise.all(allSplitPartPaths.map((partPath) => readSingleFile(partPath)));

  if (first == null) throw new Error('First part of the split GGUF file is missing');
  ...
  return {
    ...
    totalTensorInfoSize:
      first.totalTensorInfoSize == null
        ? undefined
    ...
    fullTensorInfo: first.fullTensorInfo == null ? undefined : [first, ...rest].flatMap((part) => part.fullTensorInfo ?? []),
