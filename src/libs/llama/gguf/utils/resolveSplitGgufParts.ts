...
function resolveParts(ggufPath: string) {
  ...
  if (splitPartMatch != null) {
    ...
    if (partsInfo == null) return [ggufPath];
    ...
    const res: string[] = [];
    for (let i = 1; i <= parts; i++) res.push(`${commonPath}-${String(i).padStart(5, '0')}-of-${String(parts).padStart(5, '0')}.gguf`);
...
export function getGgufSplitPartsInfo(ggufPath: string) {
  ...
  if (splitPartMatch != null) {
    ...
    if (matchLength == null || !Number.isFinite(part) || !Number.isFinite(parts) || part > parts || part === 0 || parts === 0) return null;
...
export function createSplitPartFilename(filename: string, part: number, parts: number) {
  ...
  if (filename.toLowerCase().endsWith('.gguf')) filename = filename.slice(0, -'.gguf'.length);
