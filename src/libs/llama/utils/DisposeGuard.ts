...
export class DisposeGuard {
  /** @internal */ private _preventionHandles = 0;
  /** @internal */ private _awaitingDisposeLockCallbacks: (() => void)[] = [];
  /** @internal */ private _disposeActivated = false;
  ...
  public constructor(parentDisposeGuards: DisposeGuard[] = []) {
    for (const parent of parentDisposeGuards) this._parentDisposeGuardsLocks.set(parent, null);
  }

  public addParentDisposeGuard(parent: DisposeGuard) {
    if (this._parentDisposeGuardsLocks.has(parent)) return;

    this._parentDisposeGuardsLocks.set(parent, null);

    if (this._preventionHandles > 0) this._parentDisposeGuardsLocks.set(parent, parent.createPreventDisposalHandle(true));
  }
  ...
  public async acquireDisposeLock() {
    return new Promise<void>((accept) => {
      if (this._preventionHandles > 0) this._awaitingDisposeLockCallbacks.push(accept);
      else {
  ...
  public createPreventDisposalHandle(ignoreAwaitingDispose = false) {
    if (this._isDisposeActivated() || (!ignoreAwaitingDispose && this._hasAwaitingDisposeLocks())) throw new DisposedError();
    ...
    try {
      ...
      if (this._preventionHandles === 0) this._updateParentDisposeGuardLocks();
  ...
  private _isDisposeActivated(): boolean {
    if (this._disposeActivated) return true;
  ...
  private _activateLocksIfNeeded() {
    if (this._preventionHandles > 0) return;
  ...
  /** @internal */
  private _updateParentDisposeGuardLocks(onlyAllowRemoval = false) {
    if (this._preventionHandles === 0) {
      for (const parent of this._parentDisposeGuardsLocks.keys()) {
        const parentLock = this._parentDisposeGuardsLocks.get(parent);

        if (parentLock == null) continue;
      ...
      for (const parent of this._parentDisposeGuardsLocks.keys()) {
        if (this._parentDisposeGuardsLocks.get(parent) != null) continue;
  ...
  private _hasAwaitingDisposeLocks(): boolean {
    if (this._awaitingDisposeLockCallbacks.length > 0) return true;
