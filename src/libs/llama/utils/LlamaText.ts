import type { inspect as InspectFunction, InspectOptions } from 'node:util';
...
class LlamaText {
  ...
  public joinValues(separator: LlamaText | LlamaTextValue) {
    ...
    for (let i = 0; i < this.values.length; i++) {
      ...
      if (i !== this.values.length - 1) {
        if (isLlamaText(separator)) pushAll(newValues, separator.values);
        else newValues.push(separator);
      }
    }
  ...
  public toString() {
    return this.values
      .map((value) => {
        if (value instanceof SpecialToken) return value.toString();
        if (value instanceof SpecialTokensText) return value.toString();
        return value;
      })
      .join('');
  }
  ...
  public toJSON(): LlamaTextJSON {
    if (this.values.length === 1 && typeof this.values[0] === 'string') return this.values[0];
    if (this.values.length === 0) return '';

    return this.values.map((value) => {
      if (value instanceof SpecialToken) return value.toJSON() satisfies LlamaTextJSONValue;
      if (value instanceof SpecialTokensText) return value.toJSON() satisfies LlamaTextJSONValue;
  ...
  public trimStart(): LlamaText {
    ...
    while (newValues.length > 0) {
      ...
      if (firstValue instanceof SpecialToken) break;
  ...
  public trimEnd(): LlamaText {
    ...
    while (newValues.length > 0) {
      ...
      if (lastValue instanceof SpecialToken) break;
  ...
  public includes(value: LlamaText): boolean {
    for (let i = 0; i <= this.values.length - value.values.length; i++) {
      ...
      if (!startMatch && thisValue instanceof SpecialTokensText && value.values[0] instanceof SpecialTokensText) {
        startMatch =
          value.values.length > 1 ? thisValue.value.endsWith(value.values[0].value) : thisValue.value.includes(value.values[0].value);
      }

      if (!startMatch && typeof thisValue === 'string' && typeof value.values[0] === 'string') {
        startMatch = value.values.length > 1 ? thisValue.endsWith(value.values[0]) : thisValue.includes(value.values[0]);
      }
      ...
      if (startMatch) {
        ...
        for (; j < value.values.length; j++) {
          ...
          if (!endMatch && thisValue instanceof SpecialTokensText && valueValue instanceof SpecialTokensText) {
            endMatch = value.values.length - 1 === j ? thisValue.value.startsWith(valueValue.value) : thisValue.value === valueValue.value;
          }

          if (!endMatch && typeof thisValue === 'string' && typeof valueValue === 'string') {
            endMatch = value.values.length - 1 === j ? thisValue.startsWith(valueValue) : thisValue === valueValue;
          }

          if (!endMatch) break;
        }

        if (j === value.values.length) return true;
      }
    }
  ...
  public [Symbol.for('nodejs.util.inspect.custom')](
    ...
    inspect?: typeof InspectFunction,
  ) {
    const inspectFunction = inspect ?? ((inspectOptions as any)?.inspect as undefined | typeof InspectFunction);

    if (inspectFunction == null) return JSON.stringify(this.toJSON(), undefined, 4);

    return `LlamaText(${inspectFunction(this.values, {
      ...(inspectOptions ?? {}),
      depth: depth == null ? undefined : Math.max(0, depth - 1),
    })})`;
  }
  ...
  public static fromJSON(json: LlamaTextJSON): LlamaText {
    ...
    if (typeof json === 'string') return new LlamaTextConstructor(json);

    return new LlamaTextConstructor(
      json.map((value) => {
        if (typeof value === 'string') return value;
        if (SpecialToken.isSpecialTokenJSON(value)) return SpecialToken.fromJSON(value);
        if (SpecialTokensText.isSpecialTokensTextJSON(value)) return SpecialTokensText.fromJSON(value);
  ...
  public static compare(a: LlamaText, b: LlamaText): boolean {
    ...
    if (!isLlamaText(a) || !isLlamaText(b)) return false;

    if (a.values.length !== b.values.length) return false;

    for (let i = 0; i < a.values.length; i++) {
      if (!compareLlamaTextValues(a.values[i]!, b.values[i])) return false;
    }
  ...
  public static joinValues(separator: LlamaText | string, values: readonly LlamaTextInputValue[]): LlamaText {
    ...
    for (let i = 0; i < values.length; i++) {
      ...
      if (i !== 0) newValues.push(separator);
  ...
  public static isLlamaText(value: unknown): value is LlamaText {
    ...
    if (value instanceof LlamaTextConstructor || value instanceof LlamaText) return true;
...
const LlamaTextConstructor: LlamaTextConstructor = function LlamaText(this: LlamaText, ...values: readonly LlamaTextInputValue[]) {
  ...
  if (new.target == null) return new LlamaTextConstructor(...values);
...
export class SpecialTokensText {
  ...
  public tokenize(tokenizer: Tokenizer, trimLeadingSpace = false): Token[] {
  ...
  public static fromJSON(json: LlamaTextSpecialTokensTextJSON): SpecialTokensText {
    if (SpecialTokensText.isSpecialTokensTextJSON(json)) return new SpecialTokensText(json.value);
  ...
  public static wrapIf(shouldWrap: boolean, value: string): SpecialTokensText | string {
    if (shouldWrap) return new SpecialTokensText(value);
    return value;
  }
}
...
export class SpecialToken {
  ...
  public static fromJSON(json: LlamaTextSpecialTokenJSON): SpecialToken {
    if (SpecialToken.isSpecialTokenJSON(json)) return new SpecialToken(json.value as BuiltinSpecialTokenValue);
  ...
  public static getTokenToValueMap(tokenizer: Tokenizer): ReadonlyMap<Token | undefined, BuiltinSpecialTokenValue> {
    const supportedValues = ['BOS', 'EOS', 'NL', 'EOT'] as const satisfies BuiltinSpecialTokenValue[];
    ...
    const res = new Map<Token | undefined, BuiltinSpecialTokenValue>(
      supportedValues.map((value) => [tokenizer(value, 'builtin')[0], value]),
    );
...
export function tokenizeText(text: string | LlamaText, tokenizer: Tokenizer) {
  if (typeof text === 'string') return tokenizer(text, false);
...
function createHistoryFromStringsAndValues(values: readonly LlamaTextInputValue[]): readonly LlamaTextValue[] {
  function addItemToRes(res: LlamaTextValue[], item: LlamaTextInputValue): LlamaTextValue[] {
    if (item === undefined || item === '' || (item instanceof SpecialTokensText && item.value === '')) return res;
    if (typeof item === 'string' || item instanceof SpecialTokensText || item instanceof SpecialToken) {
      res.push(item);
      return res;
    }
    if (isLlamaText(item)) {
      for (const value of item.values) res.push(value);
    ...
    if (Array.isArray(item)) {
      for (const value of item) {
        if (isLlamaText(value)) {
          for (const innerValue of value.values) res.push(innerValue);
        } else if (value === '' || (value instanceof SpecialTokensText && value.value === '')) continue;
        else if (Array.isArray(value)) addItemToRes(res, value);
        else if (typeof value === 'number' || typeof value === 'boolean') res.push(String(value));
        else res.push(value);
      }
  ...
  function squashAdjacentItems(res: LlamaTextValue[], item: LlamaTextValue) {
    ...
    if (lastItem instanceof SpecialTokensText && item instanceof SpecialTokensText) {
  ...
  return values.reduce(addItemToRes, []).reduce(squashAdjacentItems, []);
}

function compareLlamaTextValues(a?: LlamaTextValue, b?: LlamaTextValue) {
  if (a instanceof SpecialTokensText && b instanceof SpecialTokensText) return a.value === b.value;
  if (a instanceof SpecialToken && b instanceof SpecialToken) return a.value === b.value;
  if (a !== b) return false;
