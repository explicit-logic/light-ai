...
import { type LlamaText, SpecialToken, SpecialTokensText, isLlamaText } from './LlamaText';
import type { QueuedTokenRelease, QueuedTokenReleaseLock } from './TokenStreamRegulator';
...
export class StopGenerationDetector<T extends string = string> {
  ...
  /** @internal */ private _triggeredStops = new Map<
    TriggerPart<T>,
    {
  ...
  public recordGeneration({
    ...
    for (const check of currentActiveChecks) {
      ...
      if (text.length > 0) this._checkTriggerPart(check, text);
      else {
        this._activeChecks.add(check);
        checkKept = true;
      }

      if (tokens.length > 0) this._checkTriggerPart(check, tokens);
      else {
        this._activeChecks.add(check);
        checkKept = true;
      }

      if (!checkKept) check.queuedTokenReleaseLock?.dispose();
    }

    if (!startNewChecks) return;
  ...
  public addStopTrigger(stopTrigger: StopGenerationTrigger, completeEvent?: T): this {
    ...
    const triggerValues = simplifiedTrigger.flatMap((item) => {
    ...
    for (let i = 0; i < triggerValues.length; i++) {
      ...
      if (isLast) {
        ...
        if (completeEvent != null) part.completeEvents.add(completeEvent);
      } else if (part.next == null) break;
      else currentMap = part.next;
    }
  ...
  public clearTriggeredStops() {
    for (const triggeredStop of this._triggeredStops.values()) {
      for (const queuedTokenReleaseLock of triggeredStop.queuedTokenReleaseLocks) queuedTokenReleaseLock.dispose();
    }
  ...
  public clearInProgressStops() {
    for (const check of this._activeChecks) check.queuedTokenReleaseLock?.dispose();
  ...
  public getDisregardedPossibilitiesCountForAGeneration({
    ...
    if (startNewChecks) {
      const disregardedTextPossibilities =
        text.length > 0 ? this._getCountOfPossibleTriggersToBeDisregarded(this._stopTriggers.get(text[0]!), text.slice(1)) : null;
      const disregardedTokenPossibilities =
        tokens.length > 0 ? this._getCountOfPossibleTriggersToBeDisregarded(this._stopTriggers.get(tokens[0]!), tokens.slice(1)) : null;

      if (disregardedTextPossibilities != null && disregardedTokenPossibilities != null)
        res += Math.min(disregardedTextPossibilities, disregardedTokenPossibilities);
      else if (disregardedTextPossibilities != null) res += disregardedTextPossibilities;
      else if (disregardedTokenPossibilities != null) res += disregardedTokenPossibilities;
    }
  ...
  /** @internal */
  private _addFoundStop(part: TriggerPart<T>, remainingGeneration?: string | Token[], queuedTokenReleaseLock?: QueuedTokenReleaseLock) {
    ...
    if (remainingGeneration != null) triggeredStop.remainingGenerations.add(remainingGeneration);

    if (queuedTokenReleaseLock != null) triggeredStop.queuedTokenReleaseLocks.add(queuedTokenReleaseLock);
  }

  /** @internal */
  private _getCountOfPossibleTriggersToBeDisregarded(initialPart: TriggerPart<T> | undefined, value: string | Token[]) {
    if (initialPart == null) return 0;
    ...
    for (let i = 0; i < value.length && part != null; i++) {
      ...
      if (part.next == null) return res + 1;
    ...
    if (part == null || part.next == null) return res + 1;
  ...
  public static resolveStopTriggers(stopTriggers: readonly (string | Readonly<StopGenerationTrigger> | LlamaText)[], tokenizer: Tokenizer) {
    return stopTriggers
      .map((stopTrigger) => {
        if (isLlamaText(stopTrigger)) return StopGenerationDetector.resolveLlamaTextTrigger(stopTrigger, tokenizer);
        if (typeof stopTrigger === 'string') return simplifyStopTrigger([stopTrigger]);
        return simplifyStopTrigger(stopTrigger);
      })
      .filter((stopTrigger) => stopTrigger.length > 0);
  }

  public static resolveLlamaTextTrigger(llamaText: LlamaText, tokenizer: Tokenizer): StopGenerationTrigger {
    return simplifyStopTrigger(
      llamaText.values
        .filter((value) => value !== '')
        .flatMap((value) => {
          if (typeof value === 'string') return [value];
          if (value instanceof SpecialToken) return value.tokenize(tokenizer);
          if (value instanceof SpecialTokensText) return value.tokenizeSpecialTokensOnly(tokenizer);
  ...
  public static getFirstRemainingGenerationAfterStop(triggeredStops: TriggeredStop[]): {
    ...
    const [stopTrigger] = triggeredStops.filter((stopTrigger) =>
    ...
    return {
      ...
      firstRemainingGenerationAfterStop: stopTrigger?.remainingGeneration?.filter(
        (remainingGeneration) => remainingGeneration.length > 0,
      )?.[0],
    };
  }
  ...
  public static detokenizeRemainingGeneration(
    ...
    if (remainingGeneration == null || remainingGeneration.length === 0) return '';

    if (typeof remainingGeneration === 'string') return remainingGeneration;
...
function tokenizeStopTrigger(stopTrigger: StopGenerationTrigger | undefined, tokenizer: Tokenizer, specialTokens = false): Token[] {
  if (stopTrigger == null) return [];
