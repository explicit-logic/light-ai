import { DisposableHandle, DisposedError } from 'lifecycle-utils';
import type { Promisable } from './transformPromisable';
...
export class ThreadsSplitter {
  ...
  public createConsumer(wantedThreads: number, minThreads = 1) {
    if (wantedThreads !== 0 && minThreads > wantedThreads) minThreads = wantedThreads;

    if (this.maxThreads !== 0 && wantedThreads === 0) wantedThreads = this.maxThreads;

    return new ThreadsSplitterConsumer(this, wantedThreads, minThreads);
  }

  public normalizeThreadsValue(threads: number) {
    if (this.maxThreads === 0) return Math.floor(Math.max(0, threads));
  ...
  public _getUpdatedActiveThreads(inUsed: number, wanted: number, demanded: number) {
    ...
    if (inUsed > wanted) this._activeThreads -= inUsed - wanted;
  ...
  private _callOnActiveThreadsFreeIfCan(lastActiveThreads: number) {
    if (this._activeThreads >= lastActiveThreads) return;

    while (this._threadFreeCallbacks.length > 0) this._threadFreeCallbacks.shift()?.();
  }
...
export class ThreadsSplitterConsumer {
  ...
  public getAllocationToConsume(): Promisable<[threadsToUse: number, usageHandle: DisposableHandle]> {
    ...
    if (this._threadsSplitter.maxThreads === 0) return [this._wantedThreads, new DisposableHandle(() => {})];
...
class MaxNumberCollection {
  ...
  public add(number: number) {
    ...
    if (number > this._maxNumber) this._maxNumber = number;
  }
  ...
  public remove(number: number) {
    ...
    if (count === 1) {
      ...
      if (number === this._maxNumber) this._maxNumber = this._findMaxNumber();
    } else this._countMap.set(number, count - 1);
  }
