...
export class TokenStreamRegulator {
  ...
  public getPartiallyFreeChunk(tokenizer: Tokenizer) {
    if (this._queue.length > 0 && this._queue[0]!.isPartiallyFree) {
      ...
      if (queuedRelease.hasTokenLocks && !queuedRelease.hasTextLocks) {
  ...
  public getLastQueuedChunkTokens(maxTokens: number = maxRecentDetokenizerTokens) {
    ...
    for (let i = this._queue.length - 1; i >= 0 && res.length < maxTokens; i--) {
      ...
      for (let j = tokens.length - 1; j >= 0 && res.length < maxTokens; j--) res.unshift(tokens[j]!);
    }
...
export class QueuedTokenRelease {
  ...
  public getFreeTextIndex() {
    if (this._textLocks.size === 0) return this.text.length;

    return [...this._textLocks].reduce((res, lock) => Math.min(res, lock.index), this.text.length);
  }

  public getFreeTokenIndex() {
    if (this._tokenLocks.size === 0) return this.tokens.length;

    return [...this._tokenLocks].reduce((res, lock) => Math.min(res, lock.index), this.tokens.length);
  }

  public createTextIndexLock(startIndex: number) {
    const lock = QueuedTokenReleaseLock._create(startIndex, this._textLocks);

    if (startIndex >= 0 && startIndex < this.text.length) this._textLocks.add(lock);
  ...
  public createTokenIndexLock(startIndex: number) {
    ...
    if (startIndex >= 0 && startIndex < this.tokens.length) this._tokenLocks.add(lock);
...
export class QueuedTokenReleaseLock {
  ...
  public duplicate() {
    if (!this._locks.has(this)) throw new DisposedError();
