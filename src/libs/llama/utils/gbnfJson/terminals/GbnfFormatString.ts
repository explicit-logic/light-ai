...
import type { GbnfJsonFormatStringSchema } from '../types.js';
...
export class GbnfFormatString extends GbnfTerminal {
  ...
  public getGrammar(grammarGenerator: GbnfGrammarGenerator): string {
    const quote = '"\\""';
    if (this.format === 'date') return new GbnfGrammar([quote, this._getDateGrammar(), quote]).getGrammar();
    if (this.format === 'time') {
      return new GbnfGrammar([quote, this._getTimeGrammar(), quote]).getGrammar();
    }
    if (this.format === 'date-time')
      return new GbnfGrammar([quote, this._getDateGrammar(), '"T"', this._getTimeGrammar(), quote]).getGrammar();
  ...
  private _getTimeGrammar(): string {
    return new GbnfGrammar([
      ...
      '[0-5] [0-9]',
      '( "." [0-9]{3} )?',
      or(['"Z"', new GbnfGrammar([or(['"+"', '"-"']), or(['[01] [0-9]', '"2" [0-3]']), '":"', '[0-5] [0-9]']).getGrammar()]),
    ]).getGrammar();
  }
}
