...
import type { GbnfStringValue } from './GbnfStringValue.js';
...
export class GbnfObjectMap extends GbnfTerminal {
  ...
  public constructor({
    ...
    if (this.minProperties < this.fields.length) this.minProperties = this.fields.length;

    if (this.maxProperties != null && this.maxProperties < this.minProperties) this.maxProperties = this.minProperties;
    else if (this.maxProperties != null && this.maxProperties < 0) this.maxProperties = 0;
  }
  ...
  public getGrammar(grammarGenerator: GbnfGrammarGenerator): string {
    ...
    for (const { key, value } of this.fields) {
      if (objectItemsGrammar.length > 0) objectItemsGrammar.push(getCommaWhitespaceRuleName(true, 'before'));

      objectItemsGrammar.push(
        new GbnfGrammar([key.getGrammar(grammarGenerator), '":"', '[ ]?', value.resolve(grammarGenerator)]).getGrammar(),
      );
    }

    if (this.additionalProperties != null) {
      const additionalPropertiesGrammar = new GbnfGrammar([
        new GbnfString().resolve(grammarGenerator),
        '":"',
        '[ ]?',
        this.additionalProperties.resolve(grammarGenerator),
      ]);
      ...
      if (this.minProperties > this.fields.length) {
        ...
        objectItemsGrammar.push(
          new GbnfRepetition({
            ...
            maxRepetitions: this.maxProperties == null ? undefined : this.maxProperties - this.fields.length,
          ...
          objectItemsGrammar.push(
            new GbnfRepetition({
              value: new GbnfGrammar(
                [getCommaWhitespaceRuleName(true, 'before'), additionalPropertiesGrammar.resolve(grammarGenerator)],
                true,
              ),
              maxRepetitions: this.maxProperties == null ? undefined : this.maxProperties - this.fields.length,
            }).getGrammar(grammarGenerator),
