...
export class GbnfOr extends GbnfTerminal {
  ...
  public constructor(values: readonly GbnfTerminal[], useRawGrammar = false) {
    super();
    this.values = values;
  ...
  public getGrammar(grammarGenerator: GbnfGrammarGenerator): string {
    const mappedValues = this.values
      .map((v) => (this.useRawGrammar ? v.getGrammar(grammarGenerator) : v.resolve(grammarGenerator)))
      .filter((value) => value !== '' && value !== grammarNoValue);

    if (mappedValues.length === 0) return grammarNoValue;
