...
export class GbnfString extends GbnfTerminal {
  ...
  public constructor({
    ...
    if (this.maxLength != null && this.maxLength < this.minLength) this.maxLength = this.minLength;
  }

  public getGrammar(grammarGenerator: GbnfGrammarGenerator): string {
    if (this.minLength === 0 && this.maxLength == null)
      return ['"\\""', `${new GbnfInsideStringChar().resolve(grammarGenerator)}*`, '"\\""'].join(' ');
    if (this.minLength === 0 && this.maxLength === 0) return '"\\"\\""';
