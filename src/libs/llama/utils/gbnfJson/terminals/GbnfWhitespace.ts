...
import type { GbnfJsonScopeState } from '../utils/GbnfJsonScopeState.js';
...
export class GbnfWhitespace extends GbnfTerminal {
  ...
  protected override getRuleName(): string {
    return reservedRuleNames.whitespace({
      newLine: this.scopeState.settings.allowNewLines ? this.newLine : false,
...
function or(definitions: string[], wrap = true) {
  if (!wrap) return definitions.join(' | ');
...
function verbatimTextRepetition(text: string, count: number) {
  ...
  if (count <= 1) return textRepetitionGrammar;

  const textRepetitionGrammarWithRepetition = `${new GbnfVerbatimText(text).getGrammar()}{${count}}`;
  if (textRepetitionGrammarWithRepetition.length < textRepetitionGrammar.length) return textRepetitionGrammarWithRepetition;
