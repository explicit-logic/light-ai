...
export type GbnfJsonSchemaToTSType<T> = Readonly<GbnfJsonBasicSchema> extends T
  ? undefined
  : undefined extends T
    ? undefined
...
type GbnfJsonBasicSchemaToType<T extends GbnfJsonBasicSchema['type']> = T extends GbnfJsonSchemaImmutableType
  ? ImmutableTypeToType<T>
  : T extends GbnfJsonSchemaImmutableType[]
    ? ImmutableTypeToType<T[number]>
    : never;

type ImmutableTypeToType<T extends GbnfJsonSchemaImmutableType> = T extends 'string'
  ? string
  : T extends 'number'
    ? number
    : T extends 'integer'
  ...
  : T['items'] extends GbnfJsonSchema
    ? MinItems extends 0
      ...
      : T['maxItems'] extends MinItems
        ...
        : [...IndexRange<MinItems, GbnfJsonSchemaToType<T['items']>>, ...GbnfJsonSchemaToType<T['items']>[]]
    : MinItems extends 0
      ? GbnfJsonAnyValue[]
      : T['maxItems'] extends MinItems
        ? IndexRange<T['maxItems'], GbnfJsonAnyValue>
        : [...IndexRange<MinItems, GbnfJsonAnyValue>, ...GbnfJsonAnyValue[]];
...
type GbnfJsonObjectSchemaToType<
  ...
  PropsMap = Props extends undefined ? {} : { -readonly [P in keyof Props]: GbnfJsonSchemaToType<Props[P]> },
...
type _IndexRange<Value extends FillType[], MaxLength extends number, FillType = number> = Value['length'] extends MaxLength
  ? Value
