...
export function validateObjectAgainstGbnfSchema<T extends GbnfJsonSchema>(object: any, schema: T): object is GbnfJsonSchemaToType<T> {
  try {
    ...
    if (err instanceof TechnicalValidationError) throw new LlamaJsonSchemaValidationError(err.message, object, schema);
...
function validateObjectWithGbnfSchema<T extends GbnfJsonSchema>(object: any, schema: T): object is GbnfJsonSchemaToType<T> {
  if (isGbnfJsonArraySchema(schema)) return validateArray(object, schema);
  if (isGbnfJsonObjectSchema(schema)) return validateObject(object, schema);
  if (isGbnfJsonOneOfSchema(schema)) return validateOneOf(object, schema);
  if (isGbnfJsonBasicStringSchema(schema)) return validateBasicString(object, schema);
  if (isGbnfJsonFormatStringSchema(schema)) return validateFormatString(object, schema);
  if (isGbnfJsonEnumSchema(schema)) return validateEnum(object, schema);
  if (isGbnfJsonConstSchema(schema)) return validateConst(object, schema);

  if (Array.isArray(schema.type)) {
    for (const type of schema.type) {
      if (validateImmutableType(object, type)) return true;
    }

    throw new TechnicalValidationError(
      `Expected one type of [${schema.type
        .map((type) => JSON.stringify(type))
        .join(', ')}] but got type "${object === null ? null : typeof object}"`,
    );
  }

  if (validateImmutableType(object, schema.type)) return true;
...
function validateArray<T extends GbnfJsonArraySchema>(object: any, schema: T): object is GbnfJsonSchemaToType<T> {
  if (!Array.isArray(object)) throw new TechnicalValidationError(`Expected an array but got "${typeof object}"`);

  const minItems = Math.max(schema.minItems ?? 0, schema.prefixItems?.length ?? 0);
  const maxItems = schema.maxItems == null ? undefined : Math.max(schema.maxItems, minItems);
  ...
  if (maxItems != null && object.length > maxItems) {
    if (minItems === maxItems) throw new TechnicalValidationError(`Expected exactly ${minItems} items but got ${object.length}`);
  ...
  if (schema.items != null) {
    for (; index < object.length; index++) res &&= validateObjectWithGbnfSchema(object[index], schema.items);
  }
...
function validateObject<T extends GbnfJsonObjectSchema>(object: any, schema: T): object is GbnfJsonSchemaToType<T> {
  if (typeof object !== 'object' || object === null) throw new TechnicalValidationError(`Expected an object but got "${typeof object}"`);
  ...
  const maxProperties = schema.maxProperties == null ? undefined : Math.max(schema.maxProperties, minProperties);
  ...
  if (extraKeys.length > 0) {
    ...
    if (schema.additionalProperties !== true) {
      for (const key of extraKeys) res &&= validateObjectWithGbnfSchema(object[key], schema.additionalProperties);
    }
  }
  ...
  for (const key of schemaKeys) res &&= validateObjectWithGbnfSchema(object[key], schema.properties![key]!);
  ...
  if (schema.additionalProperties != null && schema.additionalProperties !== false) {
    ...
    if (maxProperties != null && objectKeys.length > maxProperties) {
...
function validateOneOf<T extends GbnfJsonOneOfSchema>(object: any, schema: T): object is GbnfJsonSchemaToType<T> {
  for (const item of schema.oneOf) {
    try {
      ...
      if (err instanceof TechnicalValidationError) continue;
...
function validateBasicString<T extends GbnfJsonBasicStringSchema>(object: any, schema: T): object is GbnfJsonSchemaToType<T> {
  ...
  const maxLength = schema.maxLength == null ? undefined : Math.max(schema.maxLength, minLength);

  if (object.length < minLength) {
    if (minLength === maxLength) throw new TechnicalValidationError(`Expected exactly ${minLength} characters but got ${object.length}`);

    throw new TechnicalValidationError(`Expected at least ${minLength} characters but got ${object.length}`);
  }
  if (maxLength != null && object.length > maxLength) {
    if (minLength === maxLength) throw new TechnicalValidationError(`Expected exactly ${minLength} characters but got ${object.length}`);
...
function validateEnum<T extends GbnfJsonEnumSchema>(object: any, schema: T): object is GbnfJsonSchemaToType<T> {
  ...
  throw new TechnicalValidationError(
    `Expected one of [${schema.enum.map((item) => JSON.stringify(item)).join(', ')}] but got ${JSON.stringify(object)}`,
  );
}

function validateConst<T extends GbnfJsonConstSchema>(object: any, schema: T): object is GbnfJsonSchemaToType<T> {
  if (object === schema.const) return true;
