...
export function getTypeScriptTypeStringForGbnfJsonSchema(schema: GbnfJsonSchema): string {
  if (isGbnfJsonOneOfSchema(schema)) {
    const values = schema.oneOf.map((altSchema) => getTypeScriptTypeStringForGbnfJsonSchema(altSchema));
  ...
  if (isGbnfJsonObjectSchema(schema)) {
    ...
    const valueTypes = Object.entries(schema.properties ?? {})
      .map(([propName, propSchema]) => {
        ...
        if (keyText === '' || valueType === '') return '';

        const mapping = `${keyText}: ${valueType}`;

        const description: string[] = propSchema.description != null && propSchema.description !== '' ? [propSchema.description] : [];
        ...
        if (isGbnfJsonBasicStringSchema(propSchema)) {
          ...
          if (propSchema.format === 'date-time') propInfo.push('format: ISO 8601 date-time');
          else propInfo.push(`format: ${String(propSchema.format)}`);
          ...
          if (propSchema.maxProperties != null)
            propInfo.push(
              `maximum number of properties: ${String(Math.floor(Math.max(propSchema.maxProperties, propSchema.minProperties ?? 0, 0)))}`,
            );
        }

        if (propInfo.length > 0) description.push(propInfo.join(', '));
    ...
    const additionalPropertiesMapSyntax =
    ...
    if (valueTypes.length === 0 && additionalPropertiesMapSyntax != null) return additionalPropertiesMapSyntax;
    if (additionalPropertiesMapSyntax != null) return [knownPropertiesMapSyntax, ' & ', additionalPropertiesMapSyntax].join('');
  ...
  if (isGbnfJsonArraySchema(schema)) {
    ...
    if (schema.prefixItems != null && schema.prefixItems.length > 0) {
      ...
      const restType = schema.items != null ? getTypeScriptTypeStringForGbnfJsonSchema(schema.items) : 'any';
      ...
      if (schema.maxItems == null || schema.maxItems > valueTypes.length) valueTypes.push(`...${wrapWithParensIfNeeded(restType)}[]`);
    ...
    if (schema.items != null) {
      ...
      if (schema.minItems != null) {
        if (schema.minItems === schema.maxItems) {
          if (schema.minItems < maxTypeRepetition) return `[${`${valuesType}, `.repeat(schema.minItems).slice(0, -', '.length)}]`;
          return ['[', `${valuesType}, `.repeat(maxTypeRepetition), '...', wrapWithParensIfNeeded(valuesType), '[]', ']'].join('');
        }
        if (schema.minItems <= 0) return `${wrapWithParensIfNeeded(valuesType)}[]`;
  ...
  if (isGbnfJsonBasicSchemaIncludesType(schema, 'string')) types.push('string');

  if (isGbnfJsonBasicSchemaIncludesType(schema, 'number')) types.push('number');

  if (isGbnfJsonBasicSchemaIncludesType(schema, 'integer')) types.push('bigint');

  if (isGbnfJsonBasicSchemaIncludesType(schema, 'boolean')) types.push('boolean');

  if (isGbnfJsonBasicSchemaIncludesType(schema, 'null')) types.push('null');
