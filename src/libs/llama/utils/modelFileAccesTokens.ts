...
export async function resolveModelFileAccessTokensTryHeaders(
  ...
  if (tokens == null || !isUrl(modelUrl)) return res;
...
async function resolveHfToken(providedToken?: string) {
  ...
  if (process.env.HF_TOKEN != null) return process.env.HF_TOKEN;

  const hfHomePath = process.env.HF_HOME || path.join(process.env.XDG_CACHE_HOME || path.join(os.homedir(), '.cache'), 'huggingface');
