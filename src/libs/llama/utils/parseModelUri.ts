...
import { normalizeGgufDownloadUrl } from '../gguf/utils/normalizeGgufDownloadUrl';
import { getFilenameForBinarySplitGgufPartUrls, resolveBinarySplitGgufPartUrls } from '../gguf/utils/resolveBinarySplitGgufPartUrls';
import { getGgufSplitPartsInfo } from '../gguf/utils/resolveSplitGgufParts';
...
export function parseModelUri(urlOrUri: string, convertUrlToSupportedUri = false): ParseModelUri | null {
  if (urlOrUri.startsWith('hf:')) return parseHuggingFaceUriContent(urlOrUri.slice('hf:'.length));
  if (urlOrUri.startsWith('huggingface:')) return parseHuggingFaceUriContent(urlOrUri.slice('huggingface:'.length));
        ...
        return parseHuggingFaceUriContent(
          [
            ...
            filePath
              ...
              .join('/'),
            branch !== defaultHuggingFaceBranch ? `#${decodeURIComponent(branch)}` : '',
          ].join(''),
        );
      }
    }
...
function parseHuggingFaceUriContent(uri: string): ParseModelUri {
  ...
  if (hashIndex >= 0) {
    ...
    if (branch === '') branch = defaultHuggingFaceBranch;
  }

  const filePathParts = rest.split('/');
  const filePath = filePathParts.map((part) => encodeURIComponent(part)).join('/');

  if (!user || !model || filePath === '') throw new Error(`Invalid Hugging Face URI: ${uri}`);
  ...
  function buildFilePrefix(user: string, model: string, branch: string, pathParts: string[], filename: string) {
    ...
    if (!doesFilenameMatchExactModelName(filename, model) || branch !== defaultHuggingFaceBranch) res.push(filenamify(model));

    if (branch !== defaultHuggingFaceBranch) res.push(filenamify(branch));

    if (pathParts.length > 0) {
      if (doesFilenameMatchExactFolderName(filename, pathParts.at(-1)!)) pathParts = pathParts.slice(0, -1);

      if (pathParts.length > 0) res.push(filenamify(pathParts.join('__')));
    }
...
function resolveModelFilenameFromUrl(modelUrl: string) {
  ...
  if (Array.isArray(binarySplitPartUrls)) return getFilenameForBinarySplitGgufPartUrls(binarySplitPartUrls);
...
function doesFilenameMatchExactModelName(filename: string, modelName: string) {
  if (!modelName.toLowerCase().endsWith('-gguf') || !filename.toLowerCase().endsWith('.gguf')) return false;

  const modelNameWithoutGguf = modelName.slice(0, -'-gguf'.length);
  const filenameWithoutGguf = filename.slice(0, -'.gguf'.length);

  if (filenameWithoutGguf.toLowerCase().startsWith(modelNameWithoutGguf.toLowerCase())) return true;

  const splitPartsInfo = getGgufSplitPartsInfo(filename);
  if (splitPartsInfo == null) return false;
...
function doesFilenameMatchExactFolderName(filename: string, folderName: string) {
  ...
  if (folderName.toLowerCase() === filenameWithoutGguf.toLowerCase()) return true;
