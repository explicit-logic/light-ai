...
import type { MergeUnionTypes } from './mergeUnionTypes';
  ...
  for (let i = 0; i < templateParts.length; i++) {
    ...
    for (; partIndex < parts.length; partIndex++) {
      ...
      if (textPart.separator === part.text) {
        const previousItem = i > 0 ? templateParts[i - 1] : null;
        const nextItem = i < templateParts.length - 1 ? templateParts[i + 1] : null;

        result[part.key as Parts[number]['key']] = {
          prefix: typeof previousItem === 'string' ? previousItem : '',
          suffix: typeof nextItem === 'string' ? nextItem : '',
        };
        partIndex++;
        break;
      }
      ...
      if (part.optional !== true) {
        if (result[part.key as Parts[number]['key']] != null) throw new Error(`Template must contain exactly one "${part.text}"`);
        if (partIndex > 0) {
