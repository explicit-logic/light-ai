...
export function safeEventCallback(callback?: undefined | undefined | never, message?: string): undefined;
...
export function safeEventCallback<const Params extends any[] = any[]>(
  ...
  if ((callback as any)?.[safeCallbackSymbol] === true) return callback;
  ...
  const res = (...args: Params) => {
    try {
      ...
      if (res instanceof Promise)
        res.catch((error) => {
          if (message != null) console.error(message, error);
