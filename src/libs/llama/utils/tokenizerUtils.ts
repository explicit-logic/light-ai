import { LlamaVocabularyType } from '../bindings/types';
import type { LlamaModelTokens } from '../evaluator/LlamaModel/LlamaModel';
...
export function resolveBeginningTokenToPrepend(vocabularyType: LlamaVocabularyType, tokens: LlamaModelTokens) {
  if (vocabularyType === LlamaVocabularyType.rwkv) return null;

  if (vocabularyType === LlamaVocabularyType.wpm) return tokens.cls;

  if (tokens.shouldPrependBosToken) return tokens.bos;
...
export function resolveEndTokenToAppend(vocabularyType: LlamaVocabularyType, tokens: LlamaModelTokens) {
  ...
  if (vocabularyType === LlamaVocabularyType.wpm) return tokens.sep;

  if (tokens.shouldAppendEosToken) return tokens.eos;
