...
export function transformPromisable<T, R>(value: Promisable<T>, transformer: (value: T) => Promisable<R>): Promisable<R> {
  if (value instanceof Promise) return value.then(transformer);
...
export function transformPromisables<const Types extends readonly any[], R>(
  ...
  if (values.some((value) => value instanceof Promise)) return Promise.all(values).then(transformer);
  ...
  function iterate(): Promisable<R> {
    while (true) {
      ...
      if (shouldContinue instanceof Promise)
        return shouldContinue.then((shouldContinue): Promisable<R> => {
          if (shouldContinue) {
            const value = callback();
            if (value instanceof Promise) return value.then(() => transformPromisable(afterthought(), iterate));
      ...
      if (shouldContinue) {
        ...
        if (afterthoughtValue instanceof Promise) return afterthoughtValue.then(iterate);
